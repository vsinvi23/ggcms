...
learning_outcomes:
  - "Distinguish LLM engineering from prompt engineering and from classical ML engineering as separate, overlapping disciplines"
  - "Describe the four layers of an LLM engineering stack: prompt/context construction, orchestration, evaluation, and production operations"
  - "Explain why a prompt that works in a playground routinely fails once it meets real users, real load, and real data"
  - "Reason about the latency, cost, and reliability trade-offs that show up the moment an LLM feature ships"
  - "Apply a structured build-evaluate-operate workflow to a concrete LLM-backed feature"
...
- **What an LLM actually does, mechanically** — tokens, context windows, and autoregressive generation. If terms like "attention" or "tokenization" are unfamiliar, start with *The Transformer Architecture: Encoder-Decoder Blocks and Self-Attention*.
- **Decoding basics** — temperature, top-p, and why the same prompt can produce different outputs across calls. See *LLM Decoding Strategies: Temperature, Top-K, and Top-P (Nucleus) Sampling* if this is new.
- **Cloud, containers, and Kubernetes fundamentals** — you don't need to be a Kubernetes expert, but you should know what a deployment, a replica, and a load balancer are, since production LLM systems sit on the same infrastructure as everything else. *Why Kubernetes Exists* covers this if needed.
- **What happens underneath an LLM API call** — request routing, batching, and the prefill/decode split. *AI Infrastructure Explained* covers that layer; this article picks up one level above it, at the application and systems-engineering layer.
...
```
              ...
              |   (see AI Infrastructure       |   (own article, own stack)
              |    Explained for this layer)   |
              +---------------+----------------+
                              |
                              v
              +-------------------------------+
...
```mermaid
sequenceDiagram
    participant U as Support Agent (UI)
    participant APP as Triage App
    participant CTX as Context Builder
    participant LLM as LLM API
    participant VAL as Validator
    participant OBS as Eval / Trace Log

    U->>APP: New ticket arrives
    APP->>CTX: Build prompt (ticket + retrieved policy excerpt)
    CTX->>LLM: system+user prompt, JSON schema
    LLM-->>CTX: structured response (or malformed text)
    CTX->>VAL: validate against schema
    alt valid
        VAL->>OBS: log trace (prompt, output, model version)
        VAL->>U: show category, priority, draft reply
    else invalid (1st failure)
        VAL->>CTX: retry with corrective instruction
        CTX->>LLM: retry call
    else invalid (2nd failure)
        VAL->>OBS: log failure trace
        VAL->>U: route to human, no auto-suggestion
    end
```
...
- An **agent** generalizes this further: the model plans a sequence of actions toward a goal, potentially across many steps, adjusting its plan based on what each tool call returns (the ReAct pattern, covered in *Agentic AI: The ReAct Loop Pattern*, and the state-machine formalization in *Multi-Agent Orchestration with LangGraph*, are two concrete implementations of this idea).
...
- **LLM-as-judge** — using a separate (often stronger, or differently-configured) model call to score a candidate output against a rubric, since exact string matching is too strict for free-text quality. This technique was covered concretely in *Evaluating RAG/Agents: Automated Metrics using RAGAS and LLM-as-a-Judge*.
...
This article stayed at the level of the general stack and its production concerns. The next article in this series, *RAG From Scratch: Why Retrieval-Augmented Generation Exists*, goes deep into the context-construction problem touched on above — how retrieval actually grounds a model in real, current data instead of relying on what it memorized during training. After that, *AI Evaluation Explained: How Do You Know a Model Actually Works?* builds out the evaluation layer introduced here into a full methodology, including how to design golden datasets and interpret LLM-as-judge scores without fooling yourself.
