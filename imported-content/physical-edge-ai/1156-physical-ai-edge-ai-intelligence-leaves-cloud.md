...
subcategory: "Edge AI Foundations"
domain: "AI Systems"
level: "Intermediate"
...
learning_outcomes:
  - "Explain precisely why round-trip latency, connectivity, bandwidth, and privacy make cloud-only inference unworkable for a specific class of workloads"
  - "Distinguish physical AI from edge AI and explain how the two ideas relate without treating them as synonyms"
  - "Describe the concrete hardware constraints (compute, memory, power, thermal envelope) that shape what a model can actually do on a device"
  - "Explain the main model-compression techniques (quantization, pruning, distillation) and what each one trades away to fit a model on constrained hardware"
  - "Trace the perception-decision-action loop that physical AI systems run, and identify where cloud dependency would break it"
  - "Reason about the security and reliability implications of moving inference off a controlled server and onto a device you don't fully control"
...
This article assumes you're comfortable with the fundamentals it builds on rather than re-deriving them:

- **What a neural network inference pass actually does** — a forward pass through learned weights, producing an output from an input, and roughly what determines how expensive that pass is. If this is unfamiliar, *The Transformer Architecture* and *AI Infrastructure Explained* both build this up from first principles.
- **The basic AI infrastructure stack** — the idea that "call a model" involves request routing, a serving layer, and compute underneath, covered in *AI Infrastructure Explained: The Stack Behind Every LLM Call*. This article is, in a sense, about what happens when you can't assume that stack lives in a data center at all.
- **What quantization and parameter-efficient fine-tuning are trying to solve at a basic level** — *Fine-Tuning LLMs: Parameter-Efficient LoRA and QLoRA NF4 Quantization* introduces quantization in the context of reducing GPU memory during fine-tuning. This article extends the same underlying idea (representing weights with fewer bits) into a different context: shrinking a model enough to run entirely on a small, low-power chip.
- **Basic container/orchestration vocabulary** — familiarity with what a container and an orchestrator do, from *Why Kubernetes Exists*, is useful background for the section below on managing large fleets of edge devices, though this article does not assume deep Kubernetes internals.

We will not re-derive how a transformer computes attention, or how LoRA adapters work internally — those are covered in their own articles. This article is specifically about what changes when the same category of model has to run on a device with a fraction of a data-center GPU's compute, memory, and power budget, potentially with no network connection at all.
...
This is an overlap, not a containment — the two circles intersect rather than one sitting fully inside the other, which is what makes both exception cases below possible at once.
...
```mermaid
flowchart LR
    A[Sense<br/>cameras, lidar, IMU, sensors] --> B[Perceive<br/>object detection, segmentation, state estimation]
    B --> C[Decide<br/>planning, policy, control logic]
    C --> D[Act<br/>motors, actuators, brakes, steering]
    D -->|environment changes| A
```
...
```mermaid
sequenceDiagram
    participant Cam as Camera (edge sensor)
    participant Edge as Edge Inference Device (NPU)
    participant Arm as Robotic Diverter Arm
    participant Cloud as Cloud (async, non-critical path)

    Cam->>Edge: Frame captured (part on belt)
    Edge->>Edge: Run quantized defect-detection model (few ms)
    Edge->>Arm: Divert / pass decision
    Arm->>Arm: Actuate diverter before part reaches packaging
    Edge-->>Cloud: Batch-upload flagged frames + metadata (later, non-blocking)
    Cloud-->>Cloud: Retrain / evaluate model on new flagged cases
    Cloud-->>Edge: Periodic model update push (out-of-band, not per-frame)
```
...
- **Model weights on the device are an extraction target.** A model shipped to thousands of physical devices is, in effect, distributed to anyone who can obtain one of those devices and extract its storage. If the model itself represents meaningful business value or embeds sensitive information from its training data, protecting the on-device weights (encryption at rest, secure boot, and hardware-backed key storage where the device supports it) is a real design requirement, not an afterthought — this is one of the reasons the hardware-enforced isolation covered in *Trusted Execution Environments* is relevant here too, for edge devices with silicon that supports it.
...
This article laid out physical AI and edge AI as a category: why the cloud-only assumption breaks, what hardware constraints edge deployment imposes, and how model compression and real-time system design respond to those constraints. From here, the natural next questions are about identity and trust for the autonomous systems this article described: *AI Agent Identity: Why Agents Need Their Own Identity Model* picks up the question of how an autonomous system — whether a cloud-based agent or a physical robot making its own decisions — proves who and what it is to the rest of your infrastructure. *Trusted Execution Environments: How Hardware Enforces Data Privacy* goes deeper into the hardware-level isolation mechanisms briefly mentioned in this article's security section, for readers who want to understand exactly how a device can protect its model weights and sensor data even against an attacker with physical access.
