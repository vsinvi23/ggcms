...
graph TD
    ...
    subgraph Third Party
        GATE[Stripe Payment Gateway]
    end

    C -- "1. Submit Order (HTTPS)" --> APIG
    APIG -- "2. Authenticate Session (gRPC)" --> AUTH
    APIG -- "3. Create Order (mTLS)" --> ORD
    ORD -- "4. Save Transaction (TCP 5432)" --> db
    ORD -- "5. Process Card (mTLS)" --> PAY
    PAY -- "6. Charge API (HTTPS)" --> GATE

    %% Trust Boundaries
    style Public Internet fill:#f3f3f3,stroke:#ccc
    style Third Party fill:#f3f3f3,stroke:#ccc
    style VPC fill:#e8f4fc,stroke:#3498db,stroke-width:2px,stroke-dasharray: 5 5
```
...
```mermaid
graph LR
    STRIDE[STRIDE Threat] --> Mitigations[Engineering Controls]
    
    subgraph Protections
    Spoofing_T[Spoofing] --> mTLS[mutual TLS & SAN checks]
    Tampering_T[Tampering] --> Signatures[Digital Signatures & HMACs]
    Repudiation_T[Repudiation] --> Audit[WORM Audit Logging]
    Info_T[Information Disclosure] --> KMS[Envelope Encryption & KMS]
    DoS_T[Denial of Service] --> RateLimit[Token Bucket Rate Limiting]
    Elevate_T[Elevation of Privilege] --> AuthZ[Cryptographically Signed JWT / RBAC]
    end
```
...
| **Payment Process** | **Repudiation** | An administrator claims they did not initiate a mass refund. | **Cryptographic WORM Logs:** Log all administrative mutations to an immutable, append-only destination (such as AWS CloudWatch with S3 Object Lock or GCP Cloud Storage with bucket lock) using structured JSON signed with the administrator's asymmetric key. |
| **Databases / Logs** | **Information Disclosure** | Attackers or developers read raw database columns containing sensitive card details. | **Envelope Encryption with KMS:** Encrypt card details *at the application layer* before database insertion using a local Data Encryption Key (DEK). Encrypt the DEK with a Key Encryption Key (KEK) hosted in a secure Key Management Service (such as AWS KMS, HashiCorp Vault, or GCP KMS). |
...
# Let's generate an asymmetric key pair representing the Auth Service's key
...
def verify_and_authorize_token(token: str, expected_role: str) -> dict:
    """ ডাউনস্ট্রিম মাইক্রোসার্ভিসে JWT যাচাই এবং প্রমাণীকরণ। """
    try:
...
**Reality:** Firewalls operate at Layers 3 and 4 of the OSI model (IP and Port). They cannot inspect API payloads for SQL injections, SSRF, or validate if a signed JWT is valid (Layer 7). Firewalls protect the network perimeter; they do not secure the application logic.
...
To solve this, advanced DevSecOps teams use **Threat Modeling as Code (TMaC)** tools, such as `pytm`.

Using Python, you define your architecture programmatically:
...
However, it does not prove **user-to-resource authorization** (it doesn't prove that *Alice* is authorized to trigger the Payment Service to charge a specific card). Downsream services must inspect both the mTLS SAN for network identity and the JWT token to ensure the logged-in user is authorized to perform the action.
