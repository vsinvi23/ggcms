...
learning_outcomes:
  - "Explain the core Zero Trust tenets (never trust/always verify, least privilege, continuous verification, assume breach) and the session-based, human- or service-shaped assumptions they were originally built around"
  - "Identify precisely why an autonomous agent breaks those assumptions: it generates its own chained requests, with no per-action human confirmation, from a reasoning process that can be hijacked mid-session without ever presenting a new credential"
  - "Design per-action authorization for agent tool calls, and explain why it differs from the session-level or connection-level authorization used in ZTNA and human MFA"
  - "Explain why session-length credentials are the wrong shape for an agent, and design short-lived, single-use, task-scoped credentials using a token-exchange pattern"
  - "Build and reason about a policy interceptor that issues per-step tokens and performs continuous behavioral verification against an agent's declared plan"
  - "Articulate the residual risk honestly: Zero Trust narrows an agent's blast radius, it does not itself detect or prevent prompt injection"
...
- **What Zero Trust and ZTNA actually replace** — the castle-and-moat/VPN model and why implicit network trust fails. See *Beyond the Perimeter: Zero Trust Network Access (ZTNA) vs Legacy VPNs* if that's new.
- **Workload identity as a concept** — that a running process can hold a cryptographically verifiable, short-lived identity document (an SVID) instead of a static secret. See *Zero Trust Identity: Enforcing Workload ID with SPIFFE/SPIRE*.
- **OAuth 2.0 token exchange (RFC 8693)** at a working level — trading one token for a narrower one, and the `act` (actor) claim that records a delegation chain. See *OAuth for AI Agents: Token Exchange and Scoped Delegations*.
- **The basic shape of an autonomous agent loop** — goal, reasoning/planning, tool selection, tool execution, observation, next action. If that loop isn't already familiar, an agentic-AI fundamentals article (for example, one covering the ReAct reason-and-act pattern) is worth reading first — this article does not re-derive it.
- **What an agent's "identity" is**, as distinct from the human who deployed it or the LLM weights it runs on. See *AI Agent Identity: Why Agents Need Their Own Identity Model*.
...
This is a genuinely different failure mode than the one ZTNA/microsegmentation solves. Microsegmentation (see the related article on micro-segmentation and assume-breach architectures) stops a compromised agent from reaching a database it was never supposed to reach at all. Per-action authorization stops a compromised agent from misusing a database it *was* supposed to reach, by checking whether this specific call, right now, still matches what it should be doing.
...
sequenceDiagram
    ...
    participant PDP as Policy Decision Point / Token Issuer
    participant Tool as Target Tool (Cloud IAM API)

    Note over Agent: Declared plan, step 22:<br/>"read logs, then check on-call schedule"
    Agent->>PEP: propose action: read_logs(pod_id)
    PEP->>PDP: authorize(step=22, action=read_logs, plan=[...])
    PDP-->>PEP: issue single-use token (TTL=8s, scope=logs:read)
    PEP->>Tool: execute read_logs() with step-22 token
    Tool-->>PEP: log contents (contains injected instruction)
    PEP-->>Agent: observation

    Note over Agent: Reasoning hijacked by injected text.<br/>Declared plan for step 23 (before hijack): "check on-call schedule"
    Agent->>PEP: propose action: widen_security_group(sg-prod-db, 0.0.0.0/0)
    PEP->>PDP: authorize(step=23, action=widen_security_group, plan=[...])
    Note over PDP: Behavioral check: requested action does not match<br/>declared plan step 23 ("check on-call schedule").<br/>Action also outside step-22 token's scope.
    PDP-->>PEP: DENY — plan/action mismatch
    PEP-->>Agent: 403 policy violation, action blocked
    Note over PEP,PDP: Security alert logged with step id,<br/>agent identity, denied action, and plan snapshot
```
...
- **Enterprise agent gateways** — the pattern covered in this identity-access folder's related enterprise agent security architecture material — typically combine this per-step authorization layer with IAM-to-agent identity mapping and output-side data-loss-prevention scanning, because per-action authorization controls what an agent is *allowed* to do, while DLP scanning catches what an *allowed* action might still leak.
...
Per-step tokens and behavioral checks only work if every agent in a system has a clean, unambiguous identity to attach them to in the first place — and in real deployments, that's rarely one agent but a fleet of specialized sub-agents sharing infrastructure. *Non-Human Identities: Managing Credential Sprawl in Multi-Agent Swarms* picks up exactly there: how to give every agent in a multi-agent system its own attested, non-shared identity instead of letting credential sprawl quietly undermine everything this article just built.
