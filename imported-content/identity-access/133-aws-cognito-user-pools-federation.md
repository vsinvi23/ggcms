...
2. **Restrict Guest/Unauthenticated Access:** Identity Pools allow "Unauthenticated identities." Ensure this is either disabled or mapped to an IAM role with near-zero permissions (e.g., restricted strictly to a public S3 folder path).
3. **Enforce Attribute-Based Access Control (ABAC):** Use claims in the User Pool JWT to map to IAM session tags. Configure IAM policies to authorize access based on matching tags, ensuring a user can only read/write S3 files where the S3 path match their own user ID claim.
