...
learning_outcomes:
  - "Explain concretely why a shared service account and raw user-token impersonation both fail as identity models for an AI agent"
  - "Name the three things a proper agent identity model must represent: a persistent agent identity, a verifiable delegation chain back to a human or system, and a scoped, time-bound permission grant"
  - "Map those three requirements onto real primitives — a distinct client identity, an actor/delegation claim, and a down-scoped short-lived token — without inventing new protocols"
  - "Trace what happens to the confused-deputy problem and the audit trail under each of the three identity models when a request goes wrong"
  - "Reason about identity lifecycle differences that agents introduce and humans don't: ephemeral sub-agents, machine-speed credential rotation, and delegation chains that can nest more than one level deep"
...
If any of those are shaky, the prerequisite articles above cover them; this one builds on top rather than re-deriving them.
...
sequenceDiagram
    ...
    User->>App: Authenticates, requests a task
    App->>IdP: Exchange user's token + agent's own client identity<br/>for a delegated, downscoped token
    IdP->>IdP: Verify agent's own credential (client auth)
    IdP->>IdP: Verify user's authenticated session (subject)
    IdP->>IdP: Downscope to the requested task's permissions
    IdP-->>App: Issue token with subject=user, actor=agent, scope=task-specific
    App->>Agent: Hand agent the delegated token for this task
    Agent->>API: Call downstream API, presenting the delegated token
    API->>API: Validate signature, subject, actor, scope, expiry
    API-->>Agent: Return result (or deny + log if scope insufficient)
    Agent-->>App: Report result
```
...
If you've read the OAuth 2.0 Token Exchange article in this knowledge base (RFC 8693), this should look familiar: `subject_token`, `actor_token`, and a downscoped result are exactly RFC 8693's vocabulary. Agent identity isn't a new protocol — it's an application of delegation semantics that already existed, applied deliberately instead of skipped.
...
None of the three elements above require inventing new cryptography. They map onto things this knowledge base has already covered:
...
The natural next step is the deep dive into the concrete gateway architecture this article's composite model implies — how a dual-principal token is validated, and how the audit record is structured, in "AI Agent Identity: Who Is Actually Making the API Call?" From there, "Zero Trust for AI Agents" extends the same delegation-and-scoping logic into a full assume-breach architecture: what changes when you stop trusting the network perimeter entirely and treat every agent call, internal or external, as something that has to prove its identity and its scope on every hop.
