...
subcategory: "AI-Native Engineering"
...
learning_outcomes:
  - "Explain why generating correct-looking code and designing sound software architecture are structurally different tasks, not points on the same skill scale"
  - "Identify the specific reasons AI coding agents are currently much better at code generation than architecture design: feedback loops, locality, reversibility, and missing non-functional inputs"
  - "Recognize the failure signature of letting an agent invent architecture from an underspecified prompt, versus letting it implement within a human-set architecture"
  - "Turn an architectural decision into a machine-checkable constraint (an ADR plus a fitness function) that keeps an agent's output inside the intended structure"
  - "Design a division of responsibility between humans and agents for a real project, with the interfaces that let both work safely on the same codebase"
...
This analogy has a limit worth naming: software is *more* reversible than a poured foundation — you can, in principle, refactor a monolith into services or migrate a database schema. But "more reversible than concrete" is not the same as "cheap to reverse." A tenant-isolation model chosen at month one and discovered to be wrong at month twelve, with real customer data and real contracts built on top of it, costs real engineering-months and real calendar time to unwind — the reversibility is theoretical until someone actually has to do it under a deadline. Keep that qualifier in mind for everything that follows: this article isn't arguing architecture decisions are irreversible, only that they're expensive enough to get right upfront that treating them like disposable code-generation output is a mistake with a long fuse.

## Before We Continue

This article assumes you're already comfortable with:

- What an AI coding agent's task loop actually does — read, plan, call a tool, observe, decide the next action (see *AI Coding Agents Explained*).
- The layered architecture an agent runs inside (orchestrator, LLM core, memory, tool registry) and why that architecture exists (see *The Architecture of an AI Coding Agent*).
- The basic argument that senior engineers remain essential once agents write a large share of the code, and why (see *Why AI-Generated Code Still Needs Senior Engineers*).
- The Zone 1/2/3 (AI-assisted / AI-augmented / AI-native) model for how much structural authority an agent has in a given workflow (see *What Does AI-Native Software Development Actually Mean?*).

If those aren't familiar, the argument here will still make sense at a high level, but the division-of-responsibility section later assumes you already understand what "an agent implementing within a spec" concretely looks like from the inside.
...
That distinction — a decided structure to implement, versus an undecided structure to invent — is the whole article. Everything below is really elaborating on why AI agents are currently much better at the first than the second, and what follows for how a real project should actually divide the work.
...
A generated function either compiles, type-checks, and passes its tests, or it doesn't — usually within seconds, always within a CI run. That tight, unambiguous feedback loop is exactly what agent frameworks are built around: generate, run, observe the failure, retry (the same loop covered in *Designing Reliable AI Agents*, scaled from a single tool call to a whole coding task). Architecture has no equivalent oracle. "Is this the right service boundary" doesn't resolve in a test run — it resolves, if at all, over months, as the system evolves and either accommodates change gracefully along that boundary or fights it at every turn. An agent optimized to converge quickly against a checkable signal has nothing to converge against here, because the signal that matters arrives on a completely different timescale than the one it operates in.

### 2. Code generation is local. Architecture is global.

Implementing a function is mostly a local problem: given the signature, the immediate call site, and nearby code for style and convention, produce a correct body. Architecture is defined by relationships between things that are *far apart* — a decision in the payments service affects a decision in the reporting service two teams over; a data-ownership choice made in month one constrains every feature request for years. Holding that kind of long-range, cross-cutting structure in view is exactly what context windows struggle with at scale (see *Context Windows Explained* and *RAG vs. Long Context for a Codebase*), and it's compounded by something context alone can't fix: a lot of the relevant structure was never fully written down anywhere retrievable — it lives in tribal knowledge, in why a previous migration failed, in a Slack thread from eighteen months ago that nobody indexed.

### 3. Code generation is cheap to retry. Architecture is expensive to retry.

An agent that generates a wrong function pays almost nothing to regenerate it — that's the entire premise behind self-correction loops and fallback tiers (see *Designing Reliable AI Agents*). Cheap iteration is a feature when the cost of being wrong is low. It becomes a liability when applied, by habit, to decisions where being wrong is *not* cheap: choosing a database, drawing a service boundary, picking a tenancy model. An agent's default operating mode — produce a plausible answer fast, let the feedback loop correct it — is tuned for the wrong cost function when the question is architectural rather than implementational.
...
A human architect who chooses a tenancy model owns that choice — defends it in a design review, gets paged when it breaks, and carries the institutional memory of why it was made into the next decision. An agent has no persistent stake across projects (see *AI Agent Memory: Short-Term and Long-Term* on why an agent's "memory" of past decisions is engineered, not felt), no incentive structure, and nothing resembling institutional scar tissue. That's not a moral failing of the technology — it's simply a different kind of accountability than the one architecture decisions actually need, which is a specific person whose judgment can be questioned and whose reasoning can be interrogated years later when the decision is revisited.
...
Concretely, that means three things exist on a well-run AI-native project, on top of what *What Does AI-Native Software Development Actually Mean?* already covers for specs and verification gates generally:
...
An ADR that only lives in a wiki page is advisory — an agent (and, honestly, a busy human) can violate it without anyone noticing until much later. A **fitness function** is an automated check that verifies a specific architectural property holds, run in CI alongside the tests. *Why AI-Generated Code Still Needs Senior Engineers* showed one shape of this — an AST-based import linter that blocks a domain layer from importing a presentation layer. The same pattern generalizes to almost any structural decision. Here's a different shape of the same idea, checking the RLS constraint from the ADR above rather than an import boundary:
...
This is deliberately a different structural property than the layering linter in article 409 — that one guards module dependencies; this one guards a data-isolation decision — to show the same technique applies across very different kinds of architectural constraints. Whatever the decision, the pattern is the same: write the ADR, then write the smallest automated check that would have caught the specific violation you're worried about.
...
- **Architectural drift through many individually-reasonable agent PRs.** Each change looks locally sensible — a new endpoint here, a new table there — but without a human-owned structure and a fitness function catching violations, the aggregate drifts away from any coherent design. This is the same pattern *Why AI-Generated Code Still Needs Senior Engineers* describes for module boundaries, generalized to any architectural property nobody is actively enforcing.
...
**Reality:** Some of it may narrow with better models. The parts rooted in missing non-code inputs, delayed feedback loops, and accountability are structural properties of what an architectural decision *is*, not a score on a benchmark — treat any claim that a specific model "now does architecture" with the same scrutiny the Verification Note above asks for.
...
Continue to article 630, "How to Review Code Written by AI Agents," for the reviewer-side skill this article assumes as a downstream step — catching the cases where an agent's implementation, even inside a well-set architecture, still needs a human's judgment. Article 637, "What Happens to Software Architecture When Code Generation Becomes Cheap?," picks up the expert-insight thread here and follows it further: what happens to the discipline of software architecture itself once the bottleneck it used to manage disappears. Article 638, "The New Software Engineering Skill: Managing AI Agents," expands on the ADR-and-fitness-function division of labor into the broader set of skills senior engineers now need to manage agents effectively across a whole project, not just at the architecture boundary.
