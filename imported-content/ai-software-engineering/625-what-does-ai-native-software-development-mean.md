...
learning_outcomes:
  - "Distinguish AI-native software development from AI-assisted coding on structural grounds, not just tool usage"
  - "Explain what changes in the SDLC when agents are treated as first-class participants: design, planning, review, testing, and deployment"
  - "Identify the new artifacts an AI-native workflow requires that a human-only workflow does not (specs-as-contracts, agent-readable test oracles, verification gates)"
  - "Describe how team roles and review responsibilities shift when an agent can generate a full PR autonomously"
  - "Recognize the organizational and reliability risks specific to AI-native development, and the controls that address them"
...
This article is about the second path, and specifically about naming what changes structurally. If you've read the earlier articles in this series — on what a coding agent actually does when it modifies your code (401), how an agent differs from an assistant (404), and why senior engineers are still essential even when agents write most of the code (409) — you already understand agent mechanics. This article is about what happens when you stop treating that mechanic as a plugin and start treating it as a member of the system.
...
- **Tool access** to actually do the work end to end: read the codebase, write code, run tests, inspect results, and iterate — the loop described in article 401 and 406.
...
In a human-only or AI-assisted process, a requirement can be fuzzy — a ticket, a Slack thread, a half-page design doc — because a human is going to interpret it, ask clarifying questions in a standup, and fill the gaps with judgment. An LLM-based agent has no standing invitation to a standup and no persistent memory of last quarter's tribal knowledge (see article 421 on agent memory). If the spec is fuzzy, the agent will confidently fill the gaps with something plausible-sounding and wrong.
...
This mirrors the resilient-agent patterns from article 427 (fallbacks, validation, retries), but scaled up from "one LLM call inside a function" to "one PR inside a repository." The verification gate is where CI, static analysis, dependency scanning, and policy-as-code checks live — and unlike a tired human reviewer, it's exhaustive and doesn't skim. Human review still exists, but its job changes: instead of asking "is this code correct," which the gate already answered, the human asks "is this the right thing to build," "does this fit the architecture," and "what did the agent trade off that a test suite can't detect" — the kinds of judgment calls a test suite genuinely cannot make (see article 409).

The diagram above still routes every passing change through a human, which is the honest default and the version most teams should build first — it's what article 626 calls the AI-augmented tier's handback point, just with a heavier gate in front of it. The fuller AI-native form goes one step further: for narrow, well-tested categories of change (a dependency patch bump that passes the full suite and a security scan, say), the gate's "pass" *is* the merge decision, and a human is looped in only when a risk-scoring policy says the change crosses a threshold — not on every single PR. That risk-based branch, not just a stronger gate in front of a still-mandatory human step, is what actually separates AI-native from AI-augmented; see article 626 for exactly where that line sits.

> **Verification Note**
> Specific CI vendor features, agent-review-agent tooling, and any named product's exact autonomous-PR-approval capabilities are evolving quickly as of this writing (September 2026) and should be checked against that vendor's current documentation rather than assumed from this description.

### 4. Testing: The Test Suite Becomes the Oracle, So It Has to Be Trustworthy

In a human-only process, tests catch regressions but a human is still the final arbiter of "does this feature actually do what we wanted." In AI-native development, the test suite (plus the spec it encodes) is frequently the *only* check between "agent generated this" and "this shipped" for a large share of routine changes. That inverts a long-standing priority: test quality stops being a "nice to have we get to eventually" and becomes as load-bearing as the production code itself, because a weak test suite doesn't just mean bugs slip through review — it means the agent has no way to know its own work is wrong, and will confidently report success on work that silently fails downstream. Article 429 covers the mechanics of testing agents themselves; the point here is that in an AI-native SDLC, the *product's* test suite takes on that same evaluator role for every agent-authored change, not just for agent-authored agents.
...
Because verification is automated and volume is higher, AI-native teams lean harder on deployment-time controls that limit how much damage a wrong-but-passing change can do: canary releases, feature flags, progressive rollout, and fast automated rollback triggered by production telemetry rather than a human noticing a graph looks wrong. This isn't a new invention — it's standard progressive-delivery practice — but it becomes load-bearing in a way it wasn't when every change had a human's judgment behind it going into production. Observability (article 430, and article 616 in the broader series) has to answer not just "is the service healthy" but "did the thing an agent shipped an hour ago behave the way its own tests predicted it would."
...
2. An agent reads both endpoints' implementations, drafts the change, writes tests matching the acceptance criteria, runs the full test suite locally, and iterates on its own failures — the same read-modify-test-commit loop from article 406, but scoped to the entire task rather than a single function.
...
- **Supply-chain exposure from agent-introduced dependencies.** An agent solving a task the fastest way it knows how may pull in a new library without the organizational hesitation a human would apply. This connects directly to the software supply-chain risks covered elsewhere in this knowledge base (see the `software-supply-chain/` category) and is a specific reason dependency policy checks belong in the verification gate.
- **Prompt injection through the task or the codebase itself.** If task descriptions come from less-trusted sources (a customer-filed ticket, a scraped issue tracker comment) or if the agent reads files that could contain adversarial instructions, the same indirect prompt injection concerns from general agent security apply here. A verification gate checks the *diff* the agent commits — it doesn't see, and can't catch, an injected instruction that gets the agent to misuse a tool mid-task (exfiltrating a secret over the network, writing outside the task's scope, calling an unrelated API) if that action never shows up in the code it eventually writes. The actual backstop for that class of attack is least-privileged, scoped tool access plus an auditable log of every tool call the agent made — the same controls listed above — not the agent's own judgment, and not the gate either; the gate only ever gets a chance to catch what makes it into the diff.
...
Continue to article 626, "AI-Assisted vs AI-Augmented vs AI-Native Software Engineering," for a sharper, side-by-side breakdown of the three zones introduced here — including how to tell, concretely, which zone a specific team or workflow is actually operating in. From there, article 627 traces how AI coding agents reshape the software development lifecycle stage by stage, and article 630 goes deep on the mechanics of reviewing code an agent wrote, which is the practical skill this article's verification-gate discussion sets up.
