...
subcategory: "Software Development Lifecycle"
...
learning_outcomes:
  - "Map each of the seven traditional SDLC phases to the concrete points where AI coding agents act today"
  - "Explain what changes in each phase's inputs, outputs, and human checkpoints once agents participate"
  - "Identify the new risks that appear at each insertion point, from prompt injection in tickets to over-trusted automated rollbacks"
  - "Distinguish which SDLC gates remain necessarily human from which can be safely automated today"
  - "Apply a phase-by-phase framework to evaluate where your own team's SDLC can safely introduce an agent next"
...
That's not a faster developer. It's a new kind of participant showing up inside a process that was never built to expect a participant like this. The interesting question isn't "can AI write code" — you already know it can, if you've read the earlier articles in this series. The question this article answers is narrower and more useful day to day: **phase by phase, where exactly does an agent insert itself into the SDLC, and what changes about that phase when it does?**

---

## Before We Continue: The Seven Phases, Refreshed
...
This article walks each row in order. For the deep mechanics of *how* an agent reads, edits, tests, and commits code, or how a full prompt-to-production pipeline is wired, we already covered that ground — see [How AI Agents Read, Modify, Test, and Commit Code](406-how-agents-read-modify-test-commit.md) and [Agentic Software Engineering: From Prompt to Production](408-agentic-software-engineering-prompt-to-prod.md). This article stays one level up: not *how an agent executes a task*, but *how the presence of agents reshapes each phase's inputs, outputs, and human checkpoints*.
...
What's changing is that agents are taking over the work *between* gates — drafting the spec, writing the diff, generating the tests, watching the metrics — while the *gate decisions themselves* are, for now, staying stubbornly human in every organization doing this responsibly. The mental model that will serve you for the rest of this article:
...
- This is exactly where human-in-the-loop approval gates matter most, because a design decision is expensive to reverse once implementation starts. See [Human-in-the-Loop AI Architecture](426-human-in-the-loop-architecture.md) for how these approval gates are actually engineered rather than just described as "a human clicks approve."
...
This is the phase the earlier articles in this series already cover in depth — how an agent parses a codebase, generates a patch, and validates it before committing. See [AI Coding Agents Explained](401-ai-coding-agents-explained.md), [How AI Coding Agents Understand an Entire Codebase](403-ai-agents-understand-codebase.md), and [How AI Agents Read, Modify, Test, and Commit Code](406-how-agents-read-modify-test-commit.md) for those mechanics. What's new at the SDLC level, rather than the mechanical level, is *where implementation now sits relative to the rest of the process*.
...
For the deeper question of how you evaluate whether an *agent itself* behaves correctly under test — deterministic mocks, LLM-as-judge scoring, and why exact-match assertions don't work for agentic outputs — see [Testing AI Agents Like Software](429-testing-ai-agents-like-software.md). This section is about agents generating tests for *your application code*, which is a different problem with a different failure mode: over-fitting a test to whatever the code currently does.
...
sequenceDiagram
    ...
    CD->>Canary: Deploy new version to 5% of traffic
    loop Every 30 seconds
        Agent->>Metrics: Query error rate, p99 latency, saturation
        Metrics-->>Agent: Current values vs. baseline
        alt Within thresholds
            Agent->>Agent: Continue observing
        else Threshold breached
            Agent->>Canary: Trigger automatic rollback
            Agent->>Human: Page with rollback reason + evidence
        end
    end
    Agent->>Human: Recommend promotion to 100% (if healthy for N minutes)
    Human->>CD: Approve full rollout
```
...
**What stays human.** Full production promotion for anything with meaningful blast radius, and the incident-command decision during an active outage. Accountability for a production incident does not transfer to an agent no matter how much of the detection and even the rollback trigger it handled — see [Why AI-Generated Code Still Needs Senior Engineers](409-why-ai-code-needs-senior-engineers.md) for the broader version of this accountability point.
...
4. **Review.** An automated pass flags that the exemption for internal service-to-service calls isn't actually implemented — the config only checks a header that's easy to spoof from outside. A human reviewer, prompted by that flag, requests a real service-identity check instead (tying back to non-human identity concerns covered in the ai-security pillar).
...
Every insertion point above is also a new trust boundary, and the ai-security pillar of this knowledge base covers each in depth — this section is a map, not a repeat.

- **Requirements phase:** an agent that ingests ticket text, linked comments, or attached files is ingesting *untrusted input* the moment any of that content could have been written or edited by someone outside your trust boundary — a classic indirect prompt-injection surface, distinct from the direct-injection case already covered for tool-using agents generally.
- **Implementation phase:** sandboxing and least-privilege execution during agent-driven code changes are exactly the concerns covered mechanically in [How AI Agents Read, Modify, Test, and Commit Code](406-how-agents-read-modify-test-commit.md) — an agent implementation step with unrestricted network or filesystem access is a supply-chain risk regardless of how good its code output is.
- **Deployment phase:** an agent with the authority to trigger a rollback or promotion needs its *own* scoped credentials to your deployment system, not a shared human's token — see the non-human-identity and secrets-management material in the ai-security pillar for how that's modeled properly.
- **Maintenance phase:** an agent auto-merging dependency bumps needs the same supply-chain scrutiny as a human doing the same thing — a patch-version bump can still pull in a compromised package, which is exactly the scenario the software-supply-chain pillar's provenance and signing material addresses.
...
- Every new insertion point is also a new trust boundary — untrusted ticket text reaching an agent, sandboxed execution during implementation, scoped credentials for deployment actions, supply-chain scrutiny for auto-merged dependency bumps — and each deserves the depth of treatment given elsewhere in this knowledge base's ai-security and software-supply-chain material, not an afterthought here.

## What to Learn Next

- *What Happens When an AI Agent Becomes Part of Your Development Team?* — the organizational and team-structure follow-on to this article's process-level view.
- *How to Review Code Written by AI Agents* — a deeper treatment of the Phase 4 shift described above.
- *AI Agents for Dependency Upgrades* — a deeper treatment of the Phase 7 maintenance pattern described above.
