...
learning_outcomes:
  - "Design a repeatable, end-to-end workflow for AI-assisted development covering task scoping, context provisioning, agent execution, automated verification gates, and human review checkpoints"
  - "Distinguish the properties of a task that make it safe to route through this workflow from the properties that mean it shouldn't be automated yet"
  - "Build a verification gate stack (static analysis, tests, security scanning, blast-radius checks) that is proportional to risk instead of uniform"
  - "Identify the specific points where a well-designed workflow still fails in practice — vague specs, verification theater, reviewer fatigue, and context rot — and the controls that address each"
  - "Explain why a workflow, not a smarter model or a better prompt, is the unit that actually determines whether AI-assisted development is reliable at scale"
...
This isn't a story about a bad model or a careless engineer. It's a story about the absence of a *workflow* — a designed, repeatable system with explicit gates, not an accumulation of individually reasonable habits that never got assembled into something that scales. Article 638 covered the individual skill of managing an agent well: scoping, context, verification, intervention. This article is about the next level up — turning that individual skill into a team-level process with defined stages, defined gates, and defined failure points, the way continuous integration turned "write tests" from an individual habit into an enforced organizational property decades ago.
...
This article assumes you've already covered the agent-mechanics prerequisites — what an agentic coding loop actually does (401), how an agent's architecture is put together (407), and the individual skill of scoping, contextualizing, verifying, and intervening on a single delegated task (638). It also assumes familiarity with how to review an individual agent-authored diff (630) and how automated agent tests differ from classical unit tests (429). This article doesn't repeat those — it's about assembling those pieces into one team-level, repeatable process, and about what breaks when that assembly is done carelessly.
...
- The specific files or modules the task touches, retrieved deliberately rather than dumped as "the whole repo" (see article 412 on why more context isn't free, and article 413 on retrieval versus long-context trade-offs).
...
**Observability** means the workflow can see what the agent is doing while it's doing it, not just the final diff. Article 430 covers instrumenting the agent's reasoning loop with structured traces; at the workflow level, the practical requirement is that someone (or some automated watchdog) can detect, in real time, the intervention signals from article 638 — repeated identical tool calls, scope visibly expanding beyond the task's stated files, a claim of success that doesn't match an observable check. A workflow without execution-time observability only ever finds out about a runaway agent after it has already finished running away.

```mermaid
flowchart LR
    A[Scoped task + provisioned context] --> B[Sandboxed agent execution]
    B --> C{Live signal check}
    C -->|Runaway loop, scope creep,\nunverified claim| D[Stop / re-scope]
    C -->|Progressing normally| E[Agent declares completion]
    E --> F[Diff + tests + trace handed\nto verification gates]
    D --> A
```
...
| Type checking / build | Does it compile / type-check at all | Hallucinated APIs and signature mismatches, often the same failure mode from article 630's review taxonomy |
...
| Security-focused static analysis (SAST) | Injection patterns, auth bypass shapes, unsafe deserialization | Security regressions that functional tests don't exercise (see article 635) |
...
- Medium-blast-radius changes get a full read, with particular attention to the tests (per article 630's point that agent-written tests need scrutiny as a first-class review target, not a checkbox).
...
Continue to "AI Coding Agents: Productivity vs Software Quality" to see how the velocity this workflow enables trades off against quality when measured over a longer horizon, and to "What Happens to Software Architecture When Code Generation Becomes Cheap?" for how cheap generation shifts the bottleneck further upstream — into specification clarity and architectural judgment — once a workflow like this is already in place and generation itself stops being the constraint.
