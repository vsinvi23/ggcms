...
learning_outcomes:
  - "Explain why managing an AI coding agent is a distinct engineering skill, not a rebrand of coding skill or people-management skill"
  - "Scope a task so an agent is likely to succeed at it, and recognize the task properties that predict agent failure"
  - "Assemble the context an agent actually needs without under-loading it (vague task) or over-loading it (context dilution)"
  - "Choose a verification strategy proportional to a task's blast radius, instead of either blind trust or exhaustive line-by-line review"
  - "Recognize the concrete signals that call for stopping an agent mid-task versus letting it keep working"
  - "Contrast agent management with delegating to a junior engineer, naming exactly where the analogy holds and where it breaks"
...
The honest answer is "it depends entirely on what the engineer did in the twenty minutes before and the five minutes after" — and that's the whole subject of this article. The agent itself (its architecture, its tool-calling loop, its context window) was covered in articles 401 and 407. What wasn't covered, because it isn't a property of the agent at all, is the skill the *human* needs to get a good outcome out of that loop, repeatedly, across dozens of tasks a week, without either babysitting every step or rubber-stamping everything that comes back green.
...
- A junior engineer who's confused *usually* says so, or produces something visibly half-finished. An agent that's confused frequently produces something fluent, complete-looking, and confidently wrong — it doesn't have the same signal of hesitation a human shows when they're out of their depth. Article 428 covers this failure shape as the "hallucination spiral": the agent doesn't pause to ask, it fills the gap with something plausible and keeps going.
- Feedback doesn't accumulate. Tell a junior engineer "we don't do it that way here" once and it usually sticks for the rest of their tenure. Tell an agent the same thing in one session and, depending on how memory is architected for that system (see the earlier article on agent memory), it may have zero persistence into the next task unless that correction is captured somewhere the agent will actually read again — a coding standard, a system prompt, a retrieved document.
...
- **Medium-stakes, partially covered** (a new feature with tests the agent wrote itself): read the tests as carefully as the implementation, maybe more carefully — article 429 makes the point that a test suite is only as trustworthy as its own design, and an agent grading its own homework by writing weak tests that happen to pass is a real failure mode, not a hypothetical one.
...
- **Repeated identical tool calls** — the agent calling the same tool with the same arguments multiple times in a row, the infinite-tool-loop pattern from article 428. This rarely self-resolves; letting it keep running mostly burns tokens and time.
...
Continue to "How to Review Code Written by AI Agents" for a deeper, review-specific treatment of the verification sub-skill introduced here — what to look for in an agent-authored diff that differs from what you'd look for in a human-authored one. From there, "How to Build a Reliable AI-Assisted Development Workflow" ties scoping, context, verification, and intervention into a repeatable team-level process rather than an individual habit. If you haven't yet, article 625, "What Does AI-Native Software Development Actually Mean?", frames why this skill becomes load-bearing at the organizational level once agents are treated as first-class SDLC participants rather than autocomplete.
