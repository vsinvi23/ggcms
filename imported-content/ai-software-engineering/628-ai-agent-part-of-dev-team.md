...
subcategory: "AI-Native Development"
...
learning_outcomes:
  - "Explain what changes organizationally when an AI agent is given standing access to a codebase, rather than being used as an ad hoc assistant"
  - "Design an onboarding process for an agent that mirrors the access, scoping, and probation period a human hire would get"
  - "Identify the specific permissions an agent needs (repo, CI/CD, secrets, ticketing) and scope each one to least privilege"
  - "Describe the new failure modes and coordination overhead a standing agent introduces into a team's workflow"
  - "Design a review and accountability model that keeps a human owner responsible for everything an agent merges"

related:
  - "Why AI-Generated Code Still Needs Senior Engineers"
  - "MCP Security: How Agents Access Tools and Data Safely"
  ...
  - "AI Agents and Least Privilege: Scoping Access to Tools and Data"
...
This article is about that gap — the organizational and technical machinery a team needs once an agent is a de facto teammate, not the coding-agent internals covered in earlier articles in this series.

## Why This Is Different From "Using an AI Coding Assistant"

Articles like *AI Coding Agents Explained* and *How AI Agents Read, Modify, Test, and Commit Code* cover what an agent does inside a single task: reading a codebase, planning edits, running tests, and committing. That's the agent's internal loop. This article is about what happens **around** that loop once it runs unsupervised, repeatedly, against a real repository with real consequences.
...
Where the analogy breaks down: a contractor learns the team's unwritten norms over weeks and eventually needs less oversight. An agent doesn't learn across sessions unless you deliberately feed it memory (see *AI Agent Memory: Short-Term and Long-Term* for how that actually works), and it doesn't develop judgment about *when to ask for help* the way a human eventually does — it will confidently attempt the ticket that's actually a landmine with the same tone as the one that's genuinely trivial. Treat the "new contractor" model as the starting posture, not a promise the agent will graduate out of it on its own.

## Before We Continue

This article assumes you're comfortable with:
- What an AI coding agent's task loop looks like (goal → plan → tool call → observation → next action) — see *AI Coding Agents Explained*.
- How an agent reads, edits, tests, and commits code in a real repository — see *How AI Agents Read, Modify, Test, and Commit Code*.
- The basic argument for why AI-generated code still needs a human in the review path — see *Why AI-Generated Code Still Needs Senior Engineers*.

If those aren't familiar yet, this article will still make sense at a high level, but the access-control and failure-mode sections assume you already know how an agent actually executes a task, not just that it can.
...
- **What is the identity?** The agent should have its own machine identity — its own service account, its own commit author, its own audit trail — never a shared human's credentials or a generic `bot` account three different automations also use. (See *AI Agent Identity: Why Agents Need Their Own Identity Model* for why this matters beyond bookkeeping — shared identity is exactly what makes an incident unattributable.)

```mermaid
flowchart TD
    A[New agent proposed for a repo] --> B{Accountable owner named?}
    B -- No --> X[Do not provision access]
    B -- Yes --> C[Define scope: repos, ticket types, task categories]
    C --> D[Provision a dedicated machine identity]
    D --> E[Grant least-privilege permissions for that scope only]
    E --> F[Probation: human approval required on every change]
    F --> G{N weeks / M merges clean?}
    G -- No --> H[Narrow scope or revoke access]
    G -- Yes --> I[Graduate to reduced review tier<br/>still never fully unreviewed]
    I --> J[Recurring access review<br/>same cadence as human offboarding audits]
```
...
A human engineer under deadline pressure tends to cut corners in visible ways: skipped edge cases, missing tests, a rushed commit message. A reviewer who's worked with that person learns to watch for it. An agent's failure signature is different, and it's covered in depth in *Why AI Agents Fail: A Deep Dive into Agentic Failure Modes* — but three patterns matter specifically for the review gate:
...
**Two agents (or an agent and a human) working the same area create real conflicts.** If an agent and a human engineer are both assigned tickets that touch the same module in the same sprint, ordinary merge-conflict friction gets worse, because neither party is watching the other's branch the way two humans on the same team informally do by talking to each other. See *Multi-Agent Systems Explained* and *One Agent vs. Multiple Agents* for the orchestration patterns that address this when you're running more than one agent — the short version for this article is: an agent's ticket assignment needs the same collision-avoidance a human's does, and "the agent will just rebase" is not a substitute for that coordination.

**Someone has to own the "why."** When a reviewer questions a design decision in a PR, a human author can explain their reasoning in a comment thread. An agent can generate a plausible-sounding explanation after the fact, but that explanation was constructed to answer the question, not necessarily recovered from the actual reasoning that produced the code — a distinction worth remembering before treating an agent's justification as evidence rather than as another output to verify. Complex or contested design decisions still need a human who actually understands the trade-off, not just a paraphrase of it.
...
```mermaid
flowchart LR
    subgraph Identity["Identity & Accountability"]
        SA[Dedicated agent service account]
        Owner[Named human owner]
    end

    subgraph Access["Scoped Access"]
        Repo[Repo: 2 assigned services only]
        CI[CI role: run tests, no deploy]
        Sec[Secrets: sandbox env only]
        Tix[Ticketing: move to review, no close]
    end

    subgraph Work["Task Flow"]
        Queue[Agent-eligible ticket queue]
        Branch[Feature branch, never main]
        PR[Pull request]
    end

    subgraph Gate["Review Gate"]
        Human[Human reviewer / code owner]
        Checks[CI: tests + lint + security scan]
    end
    ...
    SA --> Sec
    SA --> Tix
    Owner -.accountable for.-> SA

    Queue --> Branch --> PR
    PR --> Checks
    PR --> Human
    Human -- approve --> Merge[Merge to main]
    Human -- reject/feedback --> Branch

    Merge --> Audit[Audit log:<br/>who assigned, who approved, what changed]
```
...
This article focused on the organizational scaffolding — onboarding, access, review, coordination. The next articles in this pillar go deeper on two things this one only touched: how agent-authored changes compare structurally to human-designed architecture, and how to actually review an agent's code line by line. If your team is scoping agent access to specific systems, pairing this with a closer look at agent identity and least-privilege enforcement will round out the security side of the picture.
