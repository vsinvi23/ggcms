...
```
         ...
         +------------------------+------------------------+
         |                                                 |
         v (AST Parsing)                                   v (Semantic Chunking)
+--------v-----------------------+                +--------v-----------------------+
|  Syntax Tree Graph Traverser   |                |    Vector Database & Embeds    |
| - Traces callers/callees       |                | - Stores high-level intent     |
| - Resolves class hierarchies   |                | - Searches conceptually        |
| - Finds imports and type specs |                | - Uses cosine similarity       |
+--------+-----------------------+                +--------+-----------------------+
         |                                                 |
         +------------------------+------------------------+
                                  |
                  ...
                  |  Combines semantics & topology |
                  +---------------+----------------+
                                  |
                                  v
                  +---------------v----------------+
                  |  Surgical LLM Prompt Context   |
                  +--------------------------------+
```
...
Combining vector indexing and structural syntax graphs gives agents a surgical window into large software repositories, ensuring safe modifications with minimal context usage.
