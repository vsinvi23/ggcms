...
slug: "ai-agents-documentation-generation"
...
learning_outcomes:
  - "Explain why documentation drift has persisted as an unsolved problem despite decades of tooling, and why that root cause is organizational, not technical"
  - "Describe the concrete sources an AI agent draws on to generate documentation, and which of those sources actually carry the information a given doc claim needs"
  - "Design a drift-detection pipeline that flags documentation as stale the same way a build pipeline flags a failing test, rather than trusting docs to stay accurate by convention"
  - "Distinguish the categories of documentation an agent generates reliably (reference material describing what code does) from the categories that require human verification (architecture rationale, trade-offs, and tribal knowledge describing why)"
  - "Identify the specific failure modes of agent-generated documentation — confabulated behavior, stale examples, and leaked internal detail — and the review gates that catch each one"
...
An AI agent generating documentation isn't inventing facts from nothing — a well-built one is synthesizing from a specific, bounded set of sources, and the reliability of what it produces tracks directly to which source it drew from. Understanding this list is the single most useful thing in this article, because it's what lets you predict, before you even read a generated doc, how much you should trust it.
...
In a real pipeline, `check_drift` runs as a CI step on every pull request. A `True` result doesn't block the merge automatically — it posts a comment: "this function's structure changed since its documentation was last verified; the doc may need an update," optionally with the agent's *draft* of an updated doc attached for the reviewer to accept, edit, or reject. That framing — flag and draft, never auto-merge — is deliberate, and the rest of this article explains why.

---

## What Can Go Wrong?

**Confabulated behavior.** An agent asked to document a function it has genuinely misread — because the function calls into a dynamically dispatched method it can't statically resolve, or relies on a runtime configuration flag it never saw — will still produce fluent, confident-sounding prose. It has no reliable way to say "I'm not sure what this does downstream," because nothing in its training rewards hedging over a plausible-sounding answer. This is the same fluency-without-grounding failure that shows up in [AI Agents for Security Code Review](635-ai-agents-security-code-review.md): a wrong answer stated confidently is more dangerous than an obviously incomplete one, because it doesn't prompt anyone to double-check.
...
```mermaid
flowchart LR
    A[Pull request opened] --> B[CI: run tests]
    A --> C[CI: run drift detector]
    C -->|no drift| D[Doc check passes]
    C -->|drift detected| E[Agent drafts updated doc section]
    E --> F[Draft posted as PR suggestion]
    F --> G{Human reviewer}
    G -->|accepts / edits| H[Doc merged with code]
    G -->|rejects| I[Reviewer writes doc manually]
    B --> J{All checks pass?}
    D --> J
    H --> J
    I --> J
...
Continue with *What Happens to Software Architecture When Code Generation Becomes Cheap?* to see how the same "what's cheap to automate vs. what requires a human's judgment" split from this article extends to architectural decisions themselves, and *The New Software Engineering Skill: Managing AI Agents* for how a team builds the review discipline — for docs, code, and everything in between — that keeps agent output trustworthy at scale.
