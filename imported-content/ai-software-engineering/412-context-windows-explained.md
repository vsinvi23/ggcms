...
* **FlashAttention:** Bypasses reading and writing the massive $N \times N$ attention matrix to slow GPU High Bandwidth Memory. Instead, it computes attention block-by-block in fast local SRAM using tiling, reducing memory accesses and speedups up attention from quadratic memory overhead to linear.
