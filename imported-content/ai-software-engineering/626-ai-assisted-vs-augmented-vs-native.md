...
learning_outcomes:
  - "Define AI-assisted, AI-augmented, and AI-native engineering using architectural criteria, not marketing language"
  - "Identify which tier a given tool or workflow actually belongs to from its control loop, blast radius, and human-review point"
  - "Explain what has to change in an engineering organization's systems (CI/CD, review, identity, observability) to legitimately move up a tier"
  - "Recognize common mislabeling, such as calling a bounded coding agent 'AI-native' or calling a chat-based assistant 'AI-augmented'"
  - "Evaluate the trade-offs in autonomy, blast radius, and governance overhead at each tier"
...
You've likely already read about what an AI coding agent is and how it differs from a chat assistant. This article answers a narrower but more consequential question: **given that agents exist, how much of the software delivery lifecycle should they own, and what does an organization's engineering system have to look like at each level of ownership?**
...
**"Native" is not a synonym for "more agents" or "bigger context windows."** A team that spins up five specialized agents but still requires a human to review and manually merge every single PR is running an augmented workflow with a multi-agent implementation detail — not a native one. Native describes *lifecycle participation*, not *agent count*. You'll see this exact confusion addressed directly in the Common Misconceptions section below.
...
**Where it fails:** scope creep is the characteristic failure here. An agent asked to "fix the flaky test" that instead refactors the surrounding module, changes a shared utility function's signature, or "helpfully" upgrades a dependency along the way produces a PR that's technically passing CI but has a blast radius the human reviewer didn't sign up to evaluate. This is a known agentic failure pattern — worth reading about in depth if you haven't already covered how and why agents fail and how teams build reliability into the loop with fallbacks and validation.
...
- **Agent identity and standing permissions**, not a human's credentials borrowed for a session. The agent needs its own auditable identity, scoped access, and revocable permissions — this is exactly the agent-identity and Zero Trust problem this knowledge base covers elsewhere, and it is a prerequisite for native operation, not an optional extra.
...
```mermaid
flowchart TB
    subgraph Native["AI-Native SDLC (continuous, event-driven)"]
        E["Event: issue filed / alert / CVE published"] --> T["Triage agent classifies & scopes"]
        T --> P{"Policy gate:\nrisk, confidence, blast radius"}
        P -- "low risk / high confidence" --> A["Implementation agent executes\n(bounded subtask, tool loop)"]
        A --> V["Automated verification:\ntests, evals, security scan, staged rollout"]
        V -- pass --> M["Auto-merge / auto-deploy"]
        V -- fail --> R["Rollback + escalate"]
        P -- "high risk / low confidence" --> H["Human review"]
        H -- approve --> A
        H -- reject --> C["Closed / reprioritized"]
        M --> O["Org-wide observability:\ncost, drift, failure patterns"]
        R --> O
        O -.feeds back into.-> P
    end
```
...
**Solution:** There's no single correct answer here — the value is in producing the honest inventory, not in reaching a particular tier. If you find you can't confidently answer "what's the task boundary?" for an augmented-tier tool, that's the finding: scope discipline is missing, and it's worth fixing before adding more autonomy on top of it.
...
This article drew the boundary lines; the next articles in this pillar dig into each side of them in depth: what "AI-native software development" means as an organizational and architectural commitment, how the software development lifecycle itself is being restructured around agent participation, and what changes on a team when an agent becomes a standing participant rather than an occasional tool. If you haven't yet, it's also worth revisiting how reliability, testing, and observability get engineered into agents in the first place — those are the load-bearing capabilities that make the augmented-to-native transition safe rather than reckless.
