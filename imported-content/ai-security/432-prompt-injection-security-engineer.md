...
In LLMs, however, no such privilege separation exists. System instructions (such as safety guidelines and operational rules) and untrusted user inputs are ultimately serialized into a single continuous sequence of tokens. The underlying Transformer architecture processes this stream uniformly. Each token is projected into the same high-dimensional vector space and computed via the same self-attention weights.
...
When an LLM-based system processes a request, the API typically wraps user input in a predefined template. Let us examine the vulnerability lifecycle of an agent processing an order:
...
Because the resulting string is flattened, delimiters (such as `<|im_start|>`) can be forged. This is conceptually identical to escaping quotes in SQL injection. If user input contains the delimiter sequence, they can close the "user" context and open a "system" context, overriding system instructions.

## Implementation: Insecure vs. Secure Parameter Handling

To illustrate this, consider how prompt templates are subverted, and how we can implement defensive engineering practices to mitigate control-plane hijacking.
...
Below is a robust implementation using ChatML serialization, input tokenization validation, and a non-bypassable runtime interceptor to prevent plane-escape.
...
1. **Structured Outputs:** Enforce models to output structured data (JSON schemas) rather than raw text.
2. **LLM-As-A-Guard:** Use a smaller LLM whose sole job is to evaluate if the primary agent's output has deviated from its system instructions.
