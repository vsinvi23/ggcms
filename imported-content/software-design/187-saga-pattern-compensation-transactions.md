...
sequenceDiagram
    ...
    O->>I: 2. Reserve Items
    I->>I: Local Tx: Deduct Stock
    I-->>O: Success
    
    O->>P: 3. Process Payment
    P->>P: Local Tx: Charge Card
    Note over P: INSUFFICIENT FUNDS
    P-->>O: Failure
    
    Note over O: Saga Rollback Triggered
    
    O->>I: 4. COMPENSATE: Release Items
    I->>I: Local Tx: Add Stock Back
    O->>O: 5. Update Order (Status: CANCELED)
```
...
Writing compensating transactions is much harder than it sounds. Because network calls can fail, compensation endpoints must be **strictly idempotent** and **commutative**.
...
If the payment fails, the Orchestrator calls the compensation endpoint. This endpoint must be resilient to being called multiple times (e.g., if the network drops the HTTP 200 OK response).
