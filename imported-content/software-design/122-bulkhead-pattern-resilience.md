...
```mermaid
graph TD
    Client[Incoming Requests] --> OS[Order Service - Web Threads]
    
    OS -->|Checkout| PoolA[Payment Thread Pool: Max 10]
    OS -->|Stock Check| PoolB[Inventory Thread Pool: Max 50]
    
    PoolA --> PS[Payment Service]
    PoolB --> IS[Inventory Service]
```
