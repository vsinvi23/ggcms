...
3. **Multi-Resolution Generation:** Each chunk is encoded into multiple resolution and bitrate profiles (e.g., 1080p @ 5Mbps, 720p @ 3Mbps, 480p @ 1Mbps, 360p @ 400kbps) and using multiple codecs (H.264 for legacy devices, AV1/HEVC for modern devices).
4. **Stitching & Manifest Generation:** Once all chunks are processed, the chunks are stored in an edge-accessible object store, and a master **Manifest File** (e.g., `.mpd` for DASH or `.m3u8` for HLS) is generated.
...
4. The transition is seamless because all chunk variations share identical keyframes boundaries.
