...
category: "Security"
subcategory: "Trusted Execution Environments"
domain: "Cryptography"
level: "Advanced"
...
learning_outcomes:
  - "Explain how a CPU memory-encryption engine keeps DRAM contents unreadable to software that doesn't hold the matching hardware key"
  - "Name and compare the concrete hardware structures behind Intel SGX (EPC, MRENCLAVE/MRSIGNER), AMD SEV-SNP (C-bit, ASID, RMP), Intel TDX (SEAM, TD), and Arm CCA (Realms, GPT)"
  - "Trace the measurement-to-attestation-quote pipeline and explain why the signing key never leaving the chip is what makes the quote trustworthy"
  - "Precisely enumerate what a root/hypervisor-level attacker can and cannot do against each of these mechanisms"
  - "Identify the side-channel and physical attack classes that hardware isolation alone does not close, and why attestation doesn't cover them either"
  - "Reason about the trade-offs between a process-level enclave and a whole-VM trust boundary when designing a confidential workload"
...
This article goes deeper into hardware mechanics than the general confidential-computing overview, so it assumes you've either read that overview or already have this background:

- **What problem confidential computing solves** — the "data in use" gap, and why "trust us, we won't look" isn't a cryptographic guarantee. If that's new to you, read *Confidential Computing Explained: Protecting Data in Use* first — this article builds directly on its vocabulary (TEE, attestation, measurement) without re-explaining it from scratch.
- **AES and authenticated encryption** at a working level — you don't need to derive Galois field math, but you should know what "ciphertext," "integrity tag," and "authenticated encryption" mean.
- **Virtual memory and page tables** — the idea that a hypervisor maintains a mapping from guest-physical to host-physical addresses, and that this mapping is normally a position of total power over a guest.
- **The idea of a hardware root of trust** — if you've read about HSMs, you've seen the pattern of "a key that lives in hardware and is never exposed to software" before; a TEE generalizes that pattern to an entire running workload instead of one key.
...
```mermaid
sequenceDiagram
    participant Loader as Enclave/VM Launch Sequence
    participant Engine as Measurement Engine<br/>(CPU microcode / PSP / TDX module / RMM)
    participant Root as Chip Vendor's<br/>Provisioning Root of Trust
    participant Relying as Relying Party

    Loader->>Engine: Load code + initial memory pages
    Engine->>Engine: Hash each page into a running measurement<br/>(MRENCLAVE / launch measurement / TD measurement)
    Loader->>Engine: Finalize (EINIT / equivalent) — measurement is now sealed
    Loader->>Engine: Request attestation quote
    Engine->>Root: Request signing over the measurement,<br/>using a key derived from fused hardware secrets
    Root-->>Engine: Signed quote, chaining to vendor's<br/>manufacturing-time root certificate
    Engine->>Loader: Return signed quote
    Loader->>Relying: Forward quote
    Relying->>Relying: Verify signature chain to vendor root<br/>+ check revocation + compare measurement<br/>to expected known-good hash
```
...
None of this requires trusting the hypervisor's good behavior. It requires trusting that the specific hardware structure — MEE key registers, RMP, GPT, TDX module — was manufactured correctly and hasn't been physically compromised. That's a meaningfully smaller trust surface than "the entire cloud provider's software stack and staff," but it is not zero, and the next section is about exactly where it isn't zero.
...
**What you learned:** The difference between reading about a hardware feature and confirming your own machine's actual capability flags — the same distinction that matters when a relying party checks an attestation's measurement against a known-good hash instead of just checking that *a* signature is present.

> **Verification Note**
> Exact tool names, CLI flags, and `/proc/cpuinfo` flag strings vary by kernel version, microcode version, and vendor tooling release. Treat the commands above as a starting point to explore, not a guaranteed-current reference — check your distribution's and vendor's current documentation.
...
The natural next step moves from "who can see the data while it's being processed" to "where is the data legally and technically allowed to be processed at all" — see *Data Sovereignty and Geopatriation: Engineering for Regional Data Boundaries*, which builds on the same trust-minimization foundation but applies it to jurisdictional and regulatory boundaries rather than hardware ones.
