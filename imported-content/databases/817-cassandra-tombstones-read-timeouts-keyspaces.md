...
To delete data, Cassandra writes a special marker called a **Tombstone** to the database.
...
If your application demands continuous deletes (such as a job queue or temporary session keyspace), reduce the GC grace window and adjust the compaction strategy to merge SSTables more aggressively.
...
```
Look for `Estimated tombstone drop time` and `Estimated droppable tombstones` fields in the utility output to evaluate if SSTables are ready for deletion.
