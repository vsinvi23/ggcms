...
const Auth = () => {
  ...
  const { social_login: socialLoginEnabled } = useFeatureFlags();

  const [showPassword, setShowPassword] = useState(false);

  // Show a toast if redirected back from a failed OAuth attempt.
  // The oauthError param is mapped to a safe hardcoded message — never
  // rendered raw to prevent reflected XSS / phishing via crafted URLs.
  useEffect(() => {
    const OAUTH_ERRORS: Record<string, string> = {
      access_denied:    'You declined the sign-in request.',
      invalid_grant:    'The authorisation expired. Please try again.',
    ...
    if (code) {
      const safe = OAUTH_ERRORS[code] ?? 'Sign-in failed. Please try again.';
      toast({ title: 'Sign-in Failed', description: safe, variant: 'destructive' });
    }
