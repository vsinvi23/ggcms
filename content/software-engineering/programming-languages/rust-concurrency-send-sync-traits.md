---
title: "Rust Concurrency: Deciphering the Send and Sync Traits"
description: "How Rust's compiler-enforced Send and Sync marker traits guarantee data-race-free concurrent code at compile time, why Rc and RefCell are excluded, and how to fix real E0277 errors with Arc and Mutex."
type: "ARTICLE"
...
**Why is `Rc<T>` NOT `Send`?** `Rc<T>` is a reference-counted smart pointer designed for single-threaded use. It uses a non-atomic integer to track the active count of references. If you were allowed to clone and move `Rc<T>` pointers into multiple threads, they would modify the same counter concurrently without atomic synchronization — a data race on the counter that leads to double-frees or memory leaks.
...
**Why is `RefCell<T>` NOT `Sync`?** `RefCell<T>` provides "interior mutability" under single-threaded borrow-checking rules enforced at runtime. It tracks borrows using non-atomic counters. Sharing a reference `&RefCell<T>` across threads would allow multiple threads to attempt concurrent borrows, racing on the internal borrow counters.

### Auto-Trait Derivation

`Send` and `Sync` are **auto-traits**. The compiler automatically implements them for your custom struct or enum if and only if all of its constituent fields are also `Send` and `Sync`. Conversely, if a struct contains even one field that is not `Send` (like a raw pointer `*mut u32` or an `Rc`), the entire struct is automatically flagged as not `Send`.
...
| :--- | :--- | :--- | :--- |
| `T` (primitive/pure data) | Yes | Yes | Standard primitive or pure data structure |
...
- **`Send` and `Sync`** are marker traits that compile-check thread safety without any runtime overhead — there is no vtable or dynamic check involved.
- **Auto-traits** are derived automatically by the compiler; if all members of a struct are `Send`/`Sync`, the struct is too, and a single non-conforming field poisons the whole type.
- **`Arc` and `Mutex`** act as bridges, converting non-thread-safe resources (`Rc`, `RefCell`) into shared, mutable-safe concurrent abstractions, at the cost of atomic-operation and locking overhead.
