---
title: "Go Memory Alignment: Struct Padding and CPU Word-Aligned Fetching"
description: "Why a Go struct can silently occupy more than double its fields' combined size, how CPU word alignment forces the compiler to insert padding, and how reordering fields from largest to smallest reclaims that memory."
...
To maximize memory access efficiency and prevent a double-fetching penalty (where a single variable crosses a word boundary, forcing the CPU to perform two cache reads to fetch one value), compilers enforce memory alignment rules:
...
* Struct size in Go is not the sum of field sizes — it includes compiler-inserted padding to satisfy each field's alignment requirement.
* Order struct fields from largest to smallest alignment requirement to minimize padding, especially in structs allocated in bulk (slices of millions of elements).
* Use `unsafe.Sizeof()` and `unsafe.Offsetof()` to verify actual layout rather than assuming it from field order in source.
* This matters most for structs stored in large slices or maps — a few bytes saved per element compounds into megabytes at scale, and smaller structs pack more densely into CPU cache lines.
