...
description: "How Go's per-goroutine deferred call stack, panic-driven stack unwinding, and recover actually work under the hood, including the argument-evaluation trap that catches most developers."
...
Note that `recover()` only stops a panic when it is running in the same goroutine that panicked, and only when invoked directly inside a `defer`red function. A goroutine spawned with `go worker()` that panics without its own recovery handler still crashes the entire process — recovering in `main` does not protect other goroutines.
...
Additionally, deferred closures can modify named return parameters of the outer function because they share the outer function's scope. This is how many production error-wrapping patterns work:

```go
func doWork() (err error) {
	defer func() {
		if r := recover(); r != nil {
			// Convert a panic into a returned error by mutating the named return value
...
* **Arguments Are Evaluated Instantly:** Deferred function arguments are captured immediately. Use closures to delay argument evaluation until the actual execution phase.
* **Never Panic for Normal Flow Control:** Only panic on truly unrecoverable conditions, such as standard library configuration failures during initialization. Use standard `error` returns for expected failure paths.
* **Recover Only Inside Defers, Same Goroutine:** The `recover()` function must be called directly inside a deferred function in the panicking goroutine to halt panic propagation. Executing `recover` elsewhere has no effect and returns `nil`, and it cannot rescue a different goroutine.
