---
title: "Mastering Rust's Ownership, Borrowing, and Lifetimes"
description: "How Rust achieves memory safety without a garbage collector through compile-time ownership rules, the aliasing-XOR-mutability borrowing model, and lifetime annotations that prevent dangling references."
type: "ARTICLE"
...
If you assign a heap-allocated variable to another, Rust performs a "move" rather than a deep copy. The stack metadata is duplicated, but the original variable is immediately invalidated. This prevents double-free bugs since only one owner can ever drop the heap resource.

---

## Borrowing: Shared vs. Mutable References

Copying heap resources frequently is inefficient. Rust uses borrowing — exposing references (`&T` or `&mut T`) — to share access without transferring ownership. The compile-time borrow checker manages reference lifecycle via two rules:

1. **Aliasing XOR mutability**: you may have infinite read-only (shared) references (`&T`), OR exactly one read-write (mutable) reference (`&mut T`) in a given scope — never both at once.
...
By ensuring that no write access can coexist with any other read/write references, Rust guarantees that data races are impossible at compile time — without a runtime lock or a garbage collector pause.
...
While the compiler infers lifetimes automatically in most simple functions (lifetime elision), we must write explicit generic annotations (such as `'a`) when returning references from functions with multiple reference parameters, or when storing references inside structs. This tells the compiler the precise relationship between the input parameters' lifetimes and the return value's lifetime.
...
If we attempted to destroy `raw_data` inside the inner scope while trying to read `token` outside it, the Rust compiler would immediately throw a compile error (`raw_data` does not live long enough), preventing a memory fault before the program ever runs.
...
- **Borrowing** enforces mutually exclusive access patterns (aliasing XOR mutability), ensuring safety while enabling high-performance sharing without copying.
- **Lifetimes** are compile-time constraints — not runtime values — that prevent dangling pointers, ensuring references never outlive their data source.
