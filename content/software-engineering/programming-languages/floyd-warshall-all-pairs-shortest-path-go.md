---
title: "Floyd-Warshall in Go: O(V^3) All-Pairs Shortest Path with Dynamic Programming"
description: "Implement the Floyd-Warshall all-pairs shortest path algorithm in Go using a flat, cache-friendly 1D distance matrix instead of a scattered [][]int slice-of-slices."
...
When building routing tables for transit networks or solving complex network topology problems, we often need the shortest path between *all possible pairs* of nodes simultaneously, not just from a single source. The Floyd-Warshall algorithm tackles this using Dynamic Programming. While its O(V^3) time complexity restricts it to smaller graphs (V <= 1000), its elegance, brevity, and ability to handle negative edge weights make it an essential tool.

## Memory-Level Internals of Floyd-Warshall

Floyd-Warshall operates directly on an adjacency matrix instead of an adjacency list. A 2D slice `dist[V][V]` tracks the shortest known distance between any two nodes. The algorithm uses three nested loops:
...
```text
  [][]int (scattered rows)              []int flat, row-major (contiguous)
  dist[0] --> [heap block A]             dist[0*V+0] dist[0*V+1] ... dist[0*V+V-1]
  dist[1] --> [heap block B]             dist[1*V+0] dist[1*V+1] ... dist[1*V+V-1]
  dist[2] --> [heap block C]             dist[2*V+0] dist[2*V+1] ... dist[2*V+V-1]
  (pointer chase per row, cache misses)   (single contiguous buffer, prefetcher-friendly)
```
...
* Floyd-Warshall solves all-pairs shortest path in O(V^3) time and handles negative edge weights (but not negative cycles).
* A flat `[]int` matrix indexed by `i*V+j` avoids the pointer-chasing cost of Go's `[][]int` slice-of-slices representation.
* Skipping the inner loop when `dist[i][k]` is unreachable (`INF`) is a simple, high-value pruning optimization.
