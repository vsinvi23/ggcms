...
description: "Diagnose runaway heap growth and CPU-starved goroutines in a live Go service using pprof CPU/heap profiles and runtime/trace scheduler events, walking a real leaky worker-pool anti-pattern from symptom to fix."
...
We eliminate the busy-wait by removing the `select` block and allowing the worker to block naturally on the channel read. We also prevent memory leaks by avoiding the retention of oversized slices, recycling buffers through a `sync.Pool` instead:
...
By ensuring goroutines block natively when waiting for channels and leveraging `sync.Pool` to recycle structures, you can slash CPU consumption dramatically and flatten memory allocation ramps to a predictable, bounded line.

## Key Takeaways

* Use `pprof` for continuous, low-overhead production telemetry answering "who is allocating / burning CPU"; reserve `runtime/trace` for short, targeted captures answering "why did the scheduler stall."
* A `select` with a `default` branch and no blocking case is a busy-wait — it pins a CPU core spinning instead of yielding the goroutine back to the scheduler.
* `go tool pprof -alloc_space`/`-inuse_space` on a heap profile pinpoints exactly which function is retaining memory, turning a vague "memory keeps growing" report into a specific line of code.
* The fix for both symptoms in this case study is the same idiom: block on `for task := range ch` instead of polling, and recycle buffers with `sync.Pool` instead of accumulating them in a long-lived map.
