...
description: "Why treating an AI agent as either a pure human proxy or a pure machine identity creates unchecked delegation and confused-deputy risk, and how a dual-principal composite token fixes it."
...
1. **Never let an agent silently reuse a raw human JWT downstream** — the receiving service loses the ability to tell a deliberate user action from an injection-driven one.
2. **Never let an agent operate purely as a machine principal on a user's behalf** — you lose per-user access control and open the door to horizontal privilege escalation.
3. **Bind both identities cryptographically in the token** so the delegation chain (`delegated_on_behalf_of` / `act` claims) can be validated and audited at every hop.
