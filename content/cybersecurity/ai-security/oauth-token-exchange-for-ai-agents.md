...
description: "Why client-credentials tokens and raw user JWTs both fail for AI agents, and how RFC 8693 OAuth 2.0 Token Exchange down-scopes delegated access to exactly what a task needs."
...
1. **Client Credentials tokens are too broad** for an agent acting on a specific user's behalf — the target API cannot tell which human's data is in scope.
2. **The user's raw token is too broad in the other direction** — the agent inherits every permission the user has, not just what the current task needs.
3. **RFC 8693 Token Exchange** solves both by minting a short-lived, narrowly-scoped actor token that preserves the original `sub` (subject) while cryptographically proving which agent is acting and what it was authorized to do.
