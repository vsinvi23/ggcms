---
title: "OIDC for AI Agents: Verifiable Identity Beyond OAuth Scopes"
description: "Why OAuth 2.0 access tokens can't answer 'what kind of agent is calling', and how OpenID Connect ID tokens with custom agent claims let downstream services verify model version, safety alignment, and hosting environment."
type: "ARTICLE"
...
# OIDC for AI Agents: Verifiable Identity Beyond OAuth Scopes
...
1. **OAuth answers "what can you do"; OIDC answers "what are you."** Agent-to-agent trust needs both: scoped access tokens for authorization, and ID tokens with custom claims for identity attestation.
2. **JWKS-based verification removes the need for pre-shared keys** across cloud/organizational boundaries — the gateway trusts the IdP's published public keys, not a static secret.
3. **Treat model version and safety-alignment score as first-class access-control inputs.** A gateway should reject calls from agents running unpatched or unaligned model versions exactly as it would reject an expired certificate.
