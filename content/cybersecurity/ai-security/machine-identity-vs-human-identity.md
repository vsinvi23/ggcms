...
description: "Why issuing long-lived human-style tokens to AI agents is a security gap, and how SPIFFE/SPIRE workload attestation gives agents ultra-short-lived, cryptographically attested machine identities."
...
class WorkloadAttestationService:
    ...
    def attest_and_issue_svid(
        ...
        # Sign the token using HS256 for this illustration (RS256 in production)
...
1. Never issue human-lifetime tokens (hours to days) to AI agents — a compromised token stays dangerous for as long as it stays valid.
2. Machine identity must attest to *integrity of the runtime*, not just *who requested it* — a human-centric IdP has no way to detect a tampered agent sandbox.
3. SPIFFE/SPIRE workload attestation binds an ephemeral SVID to a verified container hash and runtime environment, so credentials expire in minutes and can't be replayed after the workload is gone.
