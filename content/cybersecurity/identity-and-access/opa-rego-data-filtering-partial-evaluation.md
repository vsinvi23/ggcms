---
title: "OPA Rego Policies for API Data Filtering and Partial Evaluation"
description: "How to use Open Policy Agent's partial evaluation to compile Rego authorization policies directly into safe, parameterized SQL WHERE clauses, avoiding the N+1 authorization query problem in row-level security."
...
# OPA Rego Policies for API Data Filtering and Partial Evaluation
...
### 3. Executing the Compiled Query

The final step binds the compiled clause and parameters through a parameterized database driver — never through string formatting:

```python
import sqlite3

def fetch_authorized_documents(user_context: dict) -> list:
    sql, params = get_secure_database_query(user_context)

    if not params and "WHERE" not in sql:
        # Defensive fail-closed: an empty policy result means zero visibility
        return []

    conn = sqlite3.connect("app.db")
    cursor = conn.cursor()
    cursor.execute(sql, params)  # Driver-level parameter binding, not f-string interpolation
...
- **Test the compiler, not just the policy**: The AST-to-SQL translation layer is bespoke code with no upstream test suite of its own — cover it with unit tests that assert both the generated SQL text and the bound parameter list for representative policies, not just that a query "runs."
