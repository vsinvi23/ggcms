...
description: "How the OAuth 2.0 state parameter neutralizes login CSRF and authorization-code injection attacks by binding the authorization response to the exact browser session that initiated the flow."
type: "ARTICLE"
...
```text
Victim's Browser              Client App (Relying Party)          Authorization Server
       |                            |                                     |
       |-- 1. Click "Login with Provider" ------>|                        |
       |                            | 2. Generate random `state`          |
       |                            |    (e.g., xyz123)                   |
       |<-- 3. Set Cookie: oauth_state=xyz123 ---|                        |
       |<-- 4. Redirect to Auth Server -----------|                        |
       |                                                                  |
       |-------------------- /authorize?client_id=...&state=xyz123 ------>|
       |                                                                  | 5. Authenticate
       |                                                                  |    Victim
       |<------------------ 6. Redirect to Client App --------------------|
       |     /callback?code=9999&state=xyz123                            |
       |                            |                                     |
       |-- 7. Request with Cookie & URL param -->|                        |
       |                            | 8. Compare URL `state` ==           |
       |                            |    Cookie `state`                   |
       |                            |    Match! Proceed with token        |
       |                            |    exchange.                        |
```
...
- The `state` parameter defends against login CSRF/authorization-code injection, not against authorization-code interception on public clients — that is what PKCE is for, and modern guidance recommends using both.
- Bind `state` to the session with an `HttpOnly`, `Secure`, short-lived cookie, and compare it with a constant-time function to avoid timing side-channels.
- Never trust unsigned, attacker-manipulable data embedded in `state` for application logic — sign or encrypt any payload data riding alongside the CSRF token.
