...
description: "The difference between 'someone is physically at the device' and 'the right person is at the device' in FIDO2/WebAuthn, how the authData bit flags encode it, and why skipping UV validation on the backend enables a downgrade attack."
...
Authentication Request
        |
        v
 What does the Relying Party require?
        |
   +----+-----------------------------+
   |                                  |
   v                                  v
UP Only (User Presence)      UV Required (User Verification)
   |                                  |
   v                                  v
User taps YubiKey button     User enters PIN or scans fingerprint
   |                                  |
   v                                  v
Prevents: remote malware      Prevents: remote malware AND
triggers                      physical unauthorized access
   |                                  |
   v                                  v
Vulnerable to: physical       Provides: true multi-factor
access by unauthorized user   authentication in a single step
```
...
The first byte after the RP ID hash in `authData` contains bit flags that encode the state of the authenticator during the operation.
...
Note the ordering: the UP check is unconditional (a valid assertion must always carry UP), while the UV check is only enforced `if required_uv` — meaning the server-side policy that decided "this operation needs UV" has to be threaded through from the original request options to this validation call. If that policy is lost or hardcoded to `False` anywhere along the path, the check silently degrades to UP-only regardless of what was requested during registration.
