---
title: "WebAuthn Platform vs Roaming Authenticators: Enforcing Attachment Policy"
description: "Why WebAuthn deployments that don't verify authenticator attachment type let employees register unmanaged platform biometrics instead of mandated hardware keys, with a TypeScript registration verifier that inspects AAGUID and transports."
...
# WebAuthn Platform vs Roaming Authenticators: Enforcing Attachment Policy

## The Problem: Policy Bypass via Misconfigured WebAuthn Attachment Enforcement

WebAuthn (FIDO2) provides strong phishing-resistant authentication by shifting the burden of trust to cryptographic hardware. However, many developers implement WebAuthn without properly constraining the **authenticator attachment modality**. This creates a security gap where corporate security policies can be easily bypassed.

WebAuthn defines two distinct categories of authenticators:

1. **Platform Authenticators** — Devices bound directly to the operating system (e.g., Apple Touch ID/Face ID, Windows Hello, Android biometrics). These utilize the device's internal Trusted Platform Module (TPM) or Secure Enclave.
...
The critical point is step 5: the server receives an `authenticatorAttachment` hint and an attestation object from the browser, but nothing forces the server to actually inspect them. Without explicit server-side enforcement, a registration request from a platform biometric sails through identically to one from a mandated hardware key.

## Technical Implementation

Below is a complete, illustrative TypeScript/Node.js backend implementation for parsing and validating a WebAuthn registration response. It includes strict verification of the `authenticatorAttachment` context, parses the raw `authData`, extracts the unique **AAGUID** (Authenticator Attestation GUID), and checks it against policy.
...
Note that `authenticatorAttachment` and `transports` are hints reported by the client, not cryptographically guaranteed facts. The AAGUID, extracted from the signed attestation data, is the stronger signal — but it is only trustworthy once cross-checked against a trusted registry, which is the next section.

## Defensive Hardening Checklist

1. **Verify AAGUID against MDS:** Connect your Relying Party server to the FIDO Alliance Metadata Service (MDS) to verify that the AAGUID matches certified physical roaming hardware, rather than trusting the client-reported `authenticatorAttachment` value alone.
...
4. **Treat Client Hints as Advisory, Not Authoritative:** `transports` and `authenticatorAttachment` are useful UX signals but not security boundaries on their own — the AAGUID cross-checked against MDS is the field that actually distinguishes a certified hardware key from a software-emulated one.

## Key Takeaways

- WebAuthn's `authenticatorAttachment` field is a client-reported hint, not a cryptographic guarantee — a policy that trusts it alone can be bypassed by any client willing to lie about it.
- The AAGUID embedded in the signed `authData` is the actual verifiable signal for "which authenticator model registered this credential," but only once cross-checked against the FIDO Alliance Metadata Service.
- Enforcing `userVerification: required` and a strict, non-wildcard `rpID` closes two other common attachment-policy gaps alongside AAGUID verification.
- Never hand-roll CBOR/authData byte-offset parsing in production; use a maintained WebAuthn server library and reserve manual parsing for understanding the wire format, as this article does for illustration.
