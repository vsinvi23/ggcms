...
description: "How RFC 7523 replaces shared client secrets with signed JWT assertions for non-interactive server-to-server OAuth 2.0 authentication, including the assertion claim structure and a Python implementation."
type: "ARTICLE"
...
```text
Client (Microservice A)                 Authorization Server (IdP)
        |                                         |
        | 1. Generate RSA/ECDSA keypair            |
        | 2. Register public key (JWKS URI)  ---->  |
        |                                         |
        | 3. Build assertion JWT, sign with        |
        |    private key                           |
        |                                         |
        | 4. POST /token                            |
...
// Payload
{
  "iss": "microservice_a_client_id",
  "sub": "microservice_a_client_id",
  "aud": "https://idp.example.com/token",
  "exp": 1718290000,
  "jti": "a_unique_nonce_123"
}
```

- `iss` / `sub`: Issuer and Subject — the client itself, unless delegating on behalf of a user (see below).
- `aud`: Audience — must be the IdP's token endpoint, never a resource server.
- `exp`: Expiration — must be short, e.g. 5 minutes, to bound the assertion's validity window.
- `jti`: JWT ID — a unique nonce that prevents replay of the same assertion.
...
- RFC 7523 replaces a shared client secret with a signed JWT assertion, so the Authorization Server only ever needs to store the client's public key.
- Short `exp` and a unique `jti` per assertion make each assertion single-use and time-bound, closing the replay window that plain `client_credentials` leaves open.
- Setting `sub` to a specific user ID (rather than the client's own ID) enables controlled service-account impersonation — a pattern widely used for cloud service accounts.
