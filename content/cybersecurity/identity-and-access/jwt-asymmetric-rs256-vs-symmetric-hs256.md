...
description: "Why sharing a single HS256 secret across microservices creates a single point of total compromise, how RS256 plus a JWKS endpoint fixes it, and how to defend against the algorithm-confusion downgrade attack this migration introduces."
...
```
Authorization Server --[signs with secret X]--> JWT
                                                   |
                            +----------------------+----------------------+
                            v                                             v
                     Microservice 1                                Microservice 2
              [verifies with secret X]                       [verifies with secret X]
                            |                                             |
                            v                                             v
                        Valid?                                       Valid?
```
...
```
Authorization Server --[signs with PRIVATE key]--> JWT
        |
        +--[publishes PUBLIC key]--> JWKS Endpoint
                                          |
                            +--------------+--------------+
                            v                             v
                     Microservice 1               Microservice 2
              [fetches public key from JWKS]   [fetches public key from JWKS]
                            |                             |
...
Because RS256 verification is keyed off `kid`, you can rotate signing keys without a synchronized cutover: publish the new public key in the JWKS response *alongside* the old one, start signing new tokens with the new private key, and only remove the old public key from the JWKS response once every previously issued token under it has expired.

```
JWKS response during rotation:
{
  "keys": [
    { "kid": "key-2023-10-27", "n": "...old modulus...", ... },  // still verifies old tokens
    { "kid": "key-2024-01-15", "n": "...new modulus...", ... }   // signs new tokens
  ]
}
```

A microservice that caches JWKS responses should still enforce a reasonably short cache TTL (minutes, not hours) so a compromised key can be pulled from rotation without waiting for every cache to expire on its own schedule.
