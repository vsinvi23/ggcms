---
title: "SAML XML Bomb: Defending Against the Billion Laughs Attack"
description: "How XML entity expansion turns a sub-1KB SAML assertion into a memory-exhausting denial-of-service, and how to configure Python lxml and Java DocumentBuilderFactory parsers to reject it."
...
# SAML XML Bomb: Defending Against the Billion Laughs Attack
...
## Why This Is Especially Dangerous for SAML

Unlike a typical DoS payload sent to an arbitrary endpoint, a malicious SAML Response is posted straight to the **Assertion Consumer Service (ACS)** — the endpoint every authenticated user hits after login. A single crafted assertion can take down the exact code path every legitimate employee depends on to sign in, so the blast radius is "all SSO logins," not just one API caller.

```text
   Attacker                     Service Provider ACS Endpoint
      |                                    |
      |-- POST /saml/acs -----------------> |
      |   SAMLResponse=<base64 XML bomb>    |
      |                                     |-- XML parser expands entities
      |                                     |   10 -> 100 -> ... -> 10^9 "lol"
      |                                     |
      |                                     [ CPU 100%, RAM exhausted, process OOM-killed ]
      |                                     |
      |                          All other users' SSO logins now fail
```
...
try {
    ...
    //    including an internal entity-expansion counter limit)
...
3. **Resource Limits at the Process Level:** Even with a secured parser, run the ACS handler under a memory-and-CPU-bounded process (a container with a hard memory limit, or a request-scoped timeout) so that any parser misconfiguration fails as a contained crash of one request handler, not the whole authentication service.

## Key Takeaways

- The Billion Laughs attack abuses nested DTD entity expansion to turn a small SAML assertion into a multi-gigabyte in-memory payload.
- The SAML ACS endpoint is a uniquely high-value target for this attack because it is the one endpoint every legitimate user's login depends on.
- The fix is not sanitization — it is disabling DTD processing entirely (`disallow-doctype-decl` in Java, `resolve_entities=False` in lxml), since SAML has no legitimate use for custom entities.
- Defense-in-depth means pairing a secured parser with WAF-level rejection of `<!DOCTYPE`/`<!ENTITY` payloads and per-request resource limits.
