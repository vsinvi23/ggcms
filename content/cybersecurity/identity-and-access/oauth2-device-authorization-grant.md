---
title: "OAuth 2.0 Device Authorization Grant: Securing Smart TVs and CLI Tools"
description: "How RFC 8628's Device Authorization Grant lets input-constrained devices like CLI tools, smart TVs, and IoT appliances delegate sign-in to a secondary phone or PC, plus the user-code phishing risks and defenses."
type: "ARTICLE"
...
# OAuth 2.0 Device Authorization Grant: Securing Smart TVs and CLI Tools
...
- The Device Authorization Grant is for devices with no browser or no reasonable text input, not a general substitute for the Authorization Code Flow.
- Authentication happens on a secondary device (phone/PC); the constrained device only ever sees a device code and polls for the outcome.
- User-code phishing is the primary real-world attack — always show the requesting device's context on the approval screen, and rate-limit polling to blunt DoS attempts.
