---
title: "OAuth 2.0 Authorization Code Grant with PKCE: Protocol Flow and Security Analysis"
description: "A deep-dive into the Authorization Code Grant with PKCE (RFC 7636): the authorization code interception attack it defends against, the code_verifier/code_challenge cryptography, and the Backend-for-Frontend pattern for storing tokens safely."
type: "ARTICLE"
...
# OAuth 2.0 Authorization Code Grant with PKCE: Protocol Flow and Security Analysis
...
In this deep-dive guide, we will transition from a basic conceptual model of OAuth 2.0 to a robust, defense-in-depth implementation of the **Authorization Code Grant with PKCE (Proof Key for Code Exchange)**. Specifically, we will cover:
...
- **Confidential Clients:** Server-side applications (like a Node.js/Express or Java backend) where code execution occurs on secure, private servers. These clients can safely store a **Client Secret** to authenticate themselves to the Authorization Server.
...
```text
Public Client (React/Native)     System Browser          Authorization Server
       |                              |                          |
  1. Generate secure random           |                          |
     "code_verifier"                  |                          |
  2. Compute "code_challenge" =       |                          |
     Base64URL(SHA256(verifier))      |                          |
       ...
       |                                                          |
       |-- POST /token ------------------------------------------>|
       |   client_id, code=auth_code_abc, code_verifier=VERIFIER  |
       |                              Hash provided code_verifier  |
...
Specifies the hashing algorithm. In production, this **must always be set to `S256`**. The alternative `plain` method (where the challenge is equal to the verifier) is deprecated as it fails to protect against eavesdropping on the network and should never be used.
...
if __name__ == "__main__":
    ...
    if is_valid:
        print("[OK] Success! Access Token issued safely.")
    else:
        print("[FAIL] Security Alert! Code exchange rejected.")
```
...
> **Critical Question:** Why can't the authorization server simply skip storing the `code_challenge` in its database and verify PKCE statelessly?

### Answer

To check the PKCE exchange statelessly, the server would have to encode the `code_challenge` directly into the generated `authorization_code` (for example, as an encrypted payload or signed token). While possible, this adds cryptographic complexity. Storing the challenge linked to the session state on the authorization server is highly secure, simple to scale, and keeps the authorization code small.
