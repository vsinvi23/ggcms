...
description: "How Refresh Token Rotation issues one-time-use refresh tokens grouped into token families, so that any replay of a stolen refresh token is detected instantly and the entire compromised lineage is revoked."
type: "ARTICLE"
...
### Reference Implementation: Rotate-and-Detect Transaction (Python / SQL)
...
def rotate_refresh_token(db_conn, incoming_token: str, user_id: str):
    incoming_hash = hashlib.sha256(incoming_token.encode()).hexdigest()

    with db_conn.transaction():
        row = db_conn.execute(
            "SELECT token_family, is_used FROM refresh_tokens "
            "WHERE token_hash = %s FOR UPDATE",
            (incoming_hash,)
        ).fetchone()

        if row is None:
            raise SecurityException("Unknown refresh token presented")

        token_family, is_used = row

        if is_used:
            # Replay detected: revoke the entire family immediately
            db_conn.execute(
            ...
            raise SecurityException(
                f"Breach detected: token family {token_family} revoked"
            )

        # Mark the presented token consumed
        db_conn.execute(
            "UPDATE refresh_tokens SET is_used = TRUE WHERE token_hash = %s",
            (incoming_hash,)
        )

        # Issue a descendant token in the same family
        new_token = secrets.token_urlsafe(48)
        new_hash = hashlib.sha256(new_token.encode()).hexdigest()
        db_conn.execute(
            "INSERT INTO refresh_tokens "
            "(token_hash, token_family, parent_token, user_id, expires_at) "
            "VALUES (%s, %s, %s, %s, %s)",
            (new_hash, token_family, incoming_hash, user_id,
...
- Refresh Token Rotation makes every refresh token single-use; reuse of an already-consumed token is treated as a breach signal, not a benign retry.
- Token Families let the server revoke an entire compromised lineage in one operation instead of trying to guess which individual token was stolen.
- The rotate-and-detect check must be atomic (a single transaction with row locking) — a race between two concurrent refresh attempts is exactly the scenario this pattern exists to catch.
