...
description: "How the Authorization Code Flow keeps user credentials isolated at the Identity Provider by separating the browser front-channel from the confidential client's back-channel token exchange."
type: "ARTICLE"
...
- The Authorization Code Flow is designed for confidential clients that can hold a `client_secret` server-side.
- The front-channel (browser) only ever sees a short-lived, single-use authorization code — never the access token.
- The back-channel token exchange authenticates the client, validates the code, and binds the redirect URI before issuing tokens.
- For public clients (SPAs, mobile apps) that cannot hold a secret, this flow must be combined with PKCE — see the companion PKCE deep-dive article.
