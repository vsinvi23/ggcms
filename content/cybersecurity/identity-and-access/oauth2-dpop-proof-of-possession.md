...
description: "How DPoP (RFC 9449) turns bearer access tokens into sender-constrained tokens by binding them to a client-held private key, defeating token replay after XSS or log leakage — without requiring mTLS infrastructure."
type: "ARTICLE"
...
```text
Attacker            Vulnerable Client App           Resource Server (API)
    |                        |                              |
    |-- 1. Exploit XSS / Steal Token -------------------->   |
    |                        |                              |
    |<- 2. Exfiltrate Access Token (eyJhbG...) -----------   |
    |                                                        |
    |-- 3. GET /user/data ---------------------------------->|
    |     Authorization: Bearer eyJhbG...                    |
    |                                                        |
    |            [API only checks signature.]                |
    |            [Cannot tell who sent it.]                   |
    |                                                        |
    |<- 4. Returns Victim's Data -----------------------------|
```
...
When the client application boots up, it generates an ephemeral RSA or Elliptic Curve (EC) key pair in memory (or ideally, in a secure enclave like the WebCrypto API).
...
This header contains a specialized JWT called the **DPoP Proof**. Crucially, the client signs this DPoP Proof with its newly generated **Private Key**. The payload includes the public key (in JWK format).
...
- DPoP transforms bearer tokens into sender-constrained tokens by binding them to a client-held private key, without requiring mTLS at the network layer.
- The `cnf.jkt` claim in the access token must match the public key thumbprint in the per-request DPoP proof — this is the cryptographic link an attacker cannot forge.
- Each DPoP proof is scoped to a single HTTP method and URL, which prevents a stolen proof from being replayed against a different endpoint.
