...
description: "How to build tamper-evident audit logs using HMAC hash chaining, and enforce true immutability with cloud WORM (Write-Once-Read-Many) storage such as AWS S3 Object Lock."
type: "ARTICLE"
...
## Cloud Storage WORM Enforcement
...
By coupling cryptographic chains in the application layer with hardware/cloud-enforced compliance locks, the enterprise establishes a bulletproof, non-repudiable audit trail: even if an attacker gains root access to the application server *and* the cloud account, the HMAC secret key would need to be exfiltrated separately to forge a consistent chain, and the underlying S3 objects cannot be deleted regardless of credentials until the retention period lapses.
