...
description: "How SSRF turns a trusted, VPC-privileged application server into an attacker's proxy — with a vulnerable Node.js link-preview endpoint and a production-grade Go HTTP client that defeats DNS rebinding at the socket level."
type: "ARTICLE"
...
3. **DNS Rebinding Prevention:** Ensure the validated IP address is the exact IP used for the connection. Attackers can configure a DNS server to return a public IP during validation, but resolve to an internal IP (like `127.0.0.1`) milliseconds later during the actual fetch — the classic Time-of-Check to Time-of-Use (TOCTOU) gap.

### Robust Go Mitigation Implementation

Below is a production-ready HTTP Client wrapper in Go that safely mitigates SSRF by intercepting DNS resolution and enforcing strict IP checks on the socket level — the key structural point being that validation and connection happen inside the *same* `DialContext` call, closing the TOCTOU window entirely.
...
Because the IP validation and the actual `dialer.DialContext(ctx, network, targetAddr)` call both happen inside this single `DialContext` closure — using the resolved IP directly rather than re-resolving the hostname — there is no window between "check" and "use" for a rebinding DNS server to exploit.
