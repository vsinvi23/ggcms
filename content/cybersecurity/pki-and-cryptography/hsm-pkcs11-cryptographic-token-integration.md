---
title: "HSM Integration over PKCS#11: Secure Signing Without Exposing Key Material"
description: "A practical guide to invoking cryptographic operations on a Hardware Security Module via the PKCS#11 (Cryptoki) API, covering slots, sessions, login roles, and a production-ready Go signing client."
type: "ARTICLE"
...
3. **Thread-Safety Violations** — sharing a single session handle across concurrent OS threads without enabling standard OS locking flags during Cryptoki library initialization.
4. **Context Boundary Failures** — attempting to extract key attributes and perform cryptographic operations in software, instead of offloading the entire execution payload to the HSM.
...
1. **Pin Protection via Memory Pinning & Zeroization** — never store PINs in standard Go/Java/Python strings. String variables can remain in heap garbage collector structures indefinitely. Use `byte` arrays, read credentials directly using locked system calls (e.g., `mlock` in C/Go), and zero out memory (`copy(pin, []byte{0})`) as soon as the session is established.
