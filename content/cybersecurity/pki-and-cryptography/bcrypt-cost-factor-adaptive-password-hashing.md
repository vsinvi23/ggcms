...
description: "How bcrypt's tunable cost factor and memory-hard Blowfish key schedule defend password hashes against GPU/ASIC brute force, plus a live cost-factor migration strategy for Node.js login endpoints."
type: "ARTICLE"
...
When designing authentication systems, developers must assume their database will eventually be breached. If passwords are stored in plain text, the game is over. If they are hashed with fast algorithms like MD5 or SHA-256, the game is still over — just slightly delayed. Modern GPUs and ASICs can calculate billions of SHA-256 hashes per second, making offline dictionary and brute-force attacks trivial.
