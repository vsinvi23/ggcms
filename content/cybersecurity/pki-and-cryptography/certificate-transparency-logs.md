...
description: "How RFC 6962 Certificate Transparency turns Web PKI's blind hierarchical trust into a publicly auditable, append-only Merkle-tree ledger, with the precertificate/SCT issuance flow and the math behind Merkle consistency proofs."
type: "ARTICLE"
...
```text
 Domain Owner          Certificate Authority           CT Log              Browser
      |                        |                          |                   |
      |--1. CSR --------------->|                          |                   |
      |    (Certificate Signing |                          |                   |
      |    Request)             |                          |                   |
      |                        |--2. Submit Precertificate->|                   |
      |                        |<--3. Return SCT-------------|                   |
      |                        |  (Signed Cert Timestamp)  |                   |
      |<--4. Final X.509 Cert--|                          |                   |
      |    (embeds the SCT)    |                          |                   |
      |--5. TLS Handshake (serves Cert + SCT) ------------------------------->|
      |                        |                          |    6. Verify SCTs |
      |                        |                          |    against known, |
      |                        |                          |    trusted CT logs|
```
...
```text
    ...
    +----+----+                    +------+------+
    |         |                    |             |
  H(L1,L2)   L3               H(L1,L2)      H(L3,L4)
    |                              |             |
 +--+--+                        +--+--+       +--+--+
 L1    L2                       L1    L2      L3    L4

Auditor is handed: H(L1,L2), L3, L4
Recomputes: H(H(L1,L2), L3)  == R_3  (must match previously published root)
Recomputes: H(H(L1,L2), H(L3,L4)) == R_4 (must match newly published root)
If both match, R_4 is provably an append-only extension of R_3.
```
...
### Checking SCTs from the command line

You can inspect the embedded SCTs on any live certificate without writing code:

```bash
# Show the SCT list extension on a live certificate
openssl s_client -connect github.com:443 -servername github.com </dev/null 2>/dev/null \
  | openssl x509 -noout -text \
  | grep -A 5 "CT Precertificate SCTs"
```

A certificate with zero embedded SCTs and no SCTs delivered via a TLS extension or OCSP stapling will fail Chrome's and Safari's CT enforcement policy outright.
...
- **CT doesn't replace the CA trust hierarchy — it adds public accountability on top of it**, so a rogue issuance can no longer stay silent.
- **Every certificate needs an SCT from a trusted log to be accepted by modern browsers**, embedded either in the certificate's own extension, a TLS extension, or via OCSP stapling.
- **Consistency proofs let anyone audit a log without downloading the whole tree**, scaling logarithmically even at billions of entries.
- **Domain owners should run or subscribe to a CT monitor** for their own domains — it's the only reliable way to learn about a mis-issued certificate before an attacker uses it.
