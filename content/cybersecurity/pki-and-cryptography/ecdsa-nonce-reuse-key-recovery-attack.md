---
title: "ECDSA Nonce Reuse: How a Repeated k Value Recovers the Private Key"
description: "The algebra behind ECDSA private-key recovery from nonce reuse, a working Go proof-of-concept attack, and the RFC 6979 / Ed25519 defenses that prevent it."
type: "ARTICLE"
...
# ECDSA Nonce Reuse: How a Repeated k Value Recovers the Private Key
...
However, ECDSA is incredibly fragile under **nonce leakage or reuse**. To generate an ECDSA signature, the signer must generate a unique, cryptographically secure random value $k$ (the nonce) for every single signature. If an implementation:

1. **Reuses the same nonce $k$** across two different messages signed with the same private key, or
...
2. Compute the curve point $R = k \cdot G$.
3. Compute the x-coordinate: $r = x_R \pmod n$ (if $r = 0$, start over).
4. Compute $s = k^{-1}(z + r \cdot d) \pmod n$ (if $s = 0$, start over).

The resulting signature is the pair $(r, s)$.

### Key Extraction via Nonce Reuse

Suppose a developer signs two distinct message hashes $z_1$ and $z_2$ using the same key $d$ and the exact same nonce $k$. This yields two signatures: $(r, s_1)$ and $(r, s_2)$ — notice $r$ is identical because $R = k \cdot G$ is unchanged.
...
Once the nonce $k$ is recovered, the private key $d$ falls out via simple algebra:
...
Below is an algebraic proof-of-concept in Go that demonstrates key recovery from two signatures that reused a nonce.
...
// RecoverPrivateKey extracts the private key d given two signatures generated
...
1. **Enforce deterministic nonce generation (RFC 6979).** The absolute mitigation is RFC 6979, which derives the nonce $k$ deterministically using HMAC-SHA256 of the private key $d$ and the message hash $z$:
   $$k = \text{HMAC-SHA256}(d, z)$$
   This guarantees $k$ is unguessable, yet remains identical if — and only if — the exact same message is signed under the same key. Go's standard library (`crypto/ecdsa`) and modern Rust libraries enforce RFC 6979 by default.
2. **Migrate to Ed25519 (EdDSA).** For new protocols, prefer Ed25519 over ECDSA. EdDSA has deterministic nonce generation built directly into the mathematical specification, preventing developers from ever deploying a custom, weak PRNG loop.
3. **Harden against side-channel timing attacks.** If using variable-time scalar multiplication to compute $k \cdot G$, an attacker can measure execution times to recover bits of $k$. Always use constant-time group operations (such as point-doubling ladders) to prevent timing leakage of nonce material.
