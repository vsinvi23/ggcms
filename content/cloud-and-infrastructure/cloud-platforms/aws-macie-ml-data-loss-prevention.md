---
title: "AWS Macie: ML-Driven Data Loss Prevention for S3 Data Lakes"
description: "How Amazon Macie combines managed pattern matching with contextual machine learning to find PII and secrets hiding in S3 data lakes, and how to wire its findings into an automated EventBridge remediation pipeline."
...
```json
{
  ...
  "customDataIdentifierIds": [
    "cdi-0a1b2c3d4e5f6g7h8"
  ],
  "samplingPercentage": 100
}
```

`jobType` is either `ONE_TIME` for an ad-hoc scan or `SCHEDULED` for a recurring cadence (daily/weekly/monthly). `samplingPercentage` controls how much of each bucket gets read — dropping this below 100 trades detection completeness for scan cost.
...
A minimal Lambda remediation handler consuming this event looks like:
...
def lambda_handler(event, context):
    ...
    # Immediately quarantine the object: strip public ACLs
    s3.put_object_acl(
        Bucket=bucket,
        Key=key,
        ACL="private"
    ...
    # Tag it for the security team's follow-up review
    s3.put_object_tagging(
        Bucket=bucket,
        Key=key,
        Tagging={"TagSet": [{"Key": "macie-quarantined", "Value": "true"}]}
    )

    return {"statusCode": 200, "body": f"Quarantined s3://{bucket}/{key}"}
```
