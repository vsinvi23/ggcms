...
description: "How Google's Andromeda control plane and Hoverboard host-level data plane run on top of the Jupiter Clos optical fabric to deliver software-defined routing, firewalling, and encapsulation at hyperscale."
type: "ARTICLE"
...
2. **Hoverboard (host-level data plane)**: An optimized packet-processing engine that runs in user space on every physical host. Hoverboard programs Open vSwitch (OVS) and bypasses the Linux kernel using DPDK or Google Virtual NIC (gVNIC) drivers.
...
func (hr *HoverboardRouter) EncapsulateAndRoute(pkt Packet) ([]byte, error) {
	...
	if !exists {
		// Packet miss: route to centralized on-cloud software routers for slower resolution
...
The important structural detail is the `RoutingTable` lookup at step 1: it is a plain in-memory hash map, giving O(1) route resolution for any flow the controller has already programmed. A miss does not stall the whole data plane — it falls back to the centralized controller cluster, which resolves the route once and pushes the answer back down so every subsequent packet in that flow hits the fast path.
...
1. **Andromeda splits the network into a centralized control plane and a per-host Hoverboard data plane** — the controller programs routes once, and every host enforces them locally without a per-packet round trip to the controller.
2. **Jupiter's Clos optical fabric provides non-blocking bisection bandwidth**, which is what makes it safe for Andromeda to route any host to any other host at near-line-rate without a centralized bottleneck.
3. **The routing table is a simple hash map at the host level** — the sophistication is in how it gets populated and kept current, not in the lookup itself.
4. **VM egress bandwidth scales with vCPU count**, so network-bound workloads should be sized for throughput, not just compute.
