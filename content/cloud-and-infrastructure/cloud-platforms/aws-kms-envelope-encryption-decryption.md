---
title: "AWS KMS Envelope Encryption: KEKs, Data Keys, and Secure Decryption"
description: "Why direct AWS KMS encryption fails at scale, how envelope encryption splits work between a KMS-managed KEK and a local DEK, and a production-ready Go implementation of the full encrypt/decrypt lifecycle."
...
1. **Payload Size Limitation**: The AWS KMS `Encrypt` API has a hard limit of 4 KB (4,096 bytes) for direct data encryption. Attempting to encrypt larger assets, such as database backups, PDF documents, video files, or large JSON payloads, results in a `ValidationException`.
...
3. **KMS API Quota Exhaustion & Costs**: AWS KMS imposes strict cryptographic request rate quotas (typically 10,000 requests per second in major regions). Exceeding this quota triggers `ThrottlingException` (HTTP 400), stalling pipelines. At $0.03 per 10,000 API calls, encrypting millions of individual rows directly via KMS also incurs substantial, unnecessary costs.

If your application needs to encrypt a 500 MB video file, sending that payload over the network to KMS is computationally expensive and simply won't fit under the 4 KB ceiling. So how do you securely encrypt large objects without ever distributing your master key to application servers, where it could be compromised in a memory dump or log leak?
...
To solve these limits, we implement **Envelope Encryption**. In this pattern, data is encrypted locally using a unique symmetric **Data Encryption Key (DEK)**, while the DEK itself is encrypted using a root **Key Encryption Key (KEK)** managed by AWS KMS. In AWS terminology, the KEK is called a **Customer Master Key (CMK)** — also just referred to as a "KMS key."

Think of it like sending a secure package: you put your secret letter (plaintext data) inside a small lockbox and lock it with a unique, one-time padlock (the DEK). You then put the key to that padlock inside a highly secure armored envelope (KEK encryption) before shipping it.
...
1. **The KEK (Key Encryption Key / CMK)**: Lives safely within the HSM (Hardware Security Module) boundary of AWS KMS. It never leaves KMS in plaintext.
...
### Why Envelope Encryption Is Powerful

1. **Security of the Master Key**: The CMK never leaves the AWS KMS HSMs. Applications only ever handle temporary Data Keys, radically reducing the blast radius of a compromised instance.
2. **Extreme Performance**: Only tiny Data Keys are sent over the network to KMS. The heavy lifting of encrypting gigabytes or terabytes of data occurs locally on your application instances, which can leverage hardware-accelerated AES instruction sets (like Intel AES-NI) for near-instant encryption.
3. **Simplified Key Rotation**: If you need to rotate encryption keys (e.g., for compliance), you do not need to re-encrypt petabytes of data. You simply rotate the CMK in AWS KMS, and re-encrypt the tiny Data Keys stored alongside your files. The underlying ciphertext remains unchanged, but access is now guarded by the new CMK.
...
### Quick Reference: The Same Flow in Python (boto3)

For teams working outside Go, the same `GenerateDataKey` call looks like this:
...
plaintext_key = response['Plaintext']       # Use locally, then discard
encrypted_key = response['CiphertextBlob']  # Persist alongside the ciphertext
```

Regardless of language, the SDK call shape is identical: `GenerateDataKey` returns both a plaintext key (used once, in memory, then wiped) and an encrypted key (persisted forever alongside the ciphertext it protects).
...
* **Prefer the AWS Encryption SDK for new projects**: It automates the entire envelope workflow (key generation, framing, encryption context binding) so application teams don't hand-roll the cryptographic plumbing shown above unless they need fine-grained control over the wire format.
