...
description: "A practical guide to eliminating configuration drift and copy-paste IaC by combining reusable Terraform modules with directory-isolated environments, including module design, remote state, and CI/CD guardrails."
type: "ARTICLE"
...
2. **Duplicated Boilerplate (Violating DRY)**: Maintaining duplicated blocks for VPCs, databases, and subnets across three different environments bloats the codebase. Updating a simple security group rule requires finding and editing that resource in every single repository directory.
...
### Step 1: Create the Reusable Module (`modules/autoscaled_web/`)

This module defines an auto-scaled compute cluster with a secure Application Load Balancer. It exposes variables for configuration so every environment can call the exact same code with different inputs.
...
### Step 2: Instantiate for Production (`environments/prod/`)

Deploy the production infrastructure using the reusable module as a local relative link (or a versioned Git/registry source), overriding only the variables that differ for that environment.
...
The `dev` environment (`environments/dev/main.tf`) calls the identical module with `instance_type = "t3.micro"`, `min_size = 1`, `max_size = 2`, and its own `backend.tf` pointing at a separate dev-only state bucket. No resource block is ever duplicated — only the inputs differ.
...
* **Isolate remote state per environment**: Never let `dev` and `prod` share a state backend or IAM role — a stolen `dev` credential should never be able to read or write `prod` state.
</content>
