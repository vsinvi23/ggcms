---
title: "AWS DynamoDB Internals: Partitioning, Sort Keys, and Replication"
description: "How DynamoDB shards data across physical partitions using consistent hashing, the hard per-partition throughput limits that cause hot-key throttling, and how Paxos-based replication and Global Tables deliver consistency at scale."
...
* **The Partition Key (Hash Key):** Passed through an internal hash function to produce a 128-bit hash value. This value maps directly to a specific physical partition box.
...
*(1 RCU = one strongly consistent read of up to 4 KB per second, or two eventually consistent reads. 1 WCU = one write of up to 1 KB per second.)*
