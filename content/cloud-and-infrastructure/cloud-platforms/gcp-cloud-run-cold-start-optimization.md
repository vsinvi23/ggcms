...
description: "Where Cloud Run cold-start latency actually comes from (gVisor boot vs image pull vs app init), and how to eliminate it with Terraform-configured minimum instances, CPU-always-allocated, and startup CPU boost."
type: "ARTICLE"
...
When a GCP Cloud Run service scaled to zero receives a new request, the platform must provision a new execution environment from scratch. During this phase, the client request is held in a pending state, experiencing severe latency spikes that can range from a few hundred milliseconds to several seconds.
...
In microservices, cascading cold starts across downstream calls can degrade API latency SLAs and trigger timeout faults. To optimize user experience, platform engineers must understand the internals of Cloud Run's execution environments and apply targeted mitigation strategies.

## The Mental Model: Sandboxing with gVisor

Unlike traditional virtual machines or standard Docker containers running directly on shared host kernels, Cloud Run runs container images inside **gVisor**, an open-source, user-space security sandbox created by Google.
...
gVisor intercepts application system calls and runs them through its user-space kernel (the Sentry). This sandboxing mechanism is extremely efficient; gVisor can boot its control plane and initialize a sandbox in less than 50 milliseconds.
...
1. **The gVisor sandbox itself boots in well under 50ms** — it is essentially never the dominant cost in a Cloud Run cold start.
2. **Image pull time and application initialization dominate cold-start latency** — a 500MB image with a heavyweight framework boot (Spring Boot, Rails) can add over a second, dwarfing platform overhead.
3. **`min_instance_count` trades idle cost for eliminated cold starts** on steady-state traffic, but a scale-up event past your warm instance count still incurs one.
4. **`cpu_idle = false` and startup CPU boost address the "app init is slow" half of the problem**, while smaller images and lazy connection pooling address the "download and bootstrap is slow" half — both are needed for consistently low p99 latency.
