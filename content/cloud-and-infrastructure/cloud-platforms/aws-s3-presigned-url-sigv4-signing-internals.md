---
title: "Securing AWS S3: Inside the SigV4 Pre-Signed URL Signing Math"
description: "A deep dive into the cryptographic key-derivation chain behind AWS Signature Version 4, with a zero-dependency Python implementation that manually signs an S3 pre-signed URL."
type: "ARTICLE"
...
# Securing AWS S3: Inside the SigV4 Pre-Signed URL Signing Math
...
- **The Public Bucket Fallback**: Opening S3 bucket permissions to the public bypasses all authorization controls, making private corporate data searchable and downloadable by any web crawler.
...
Each derivation step narrows the scope of what the resulting key can sign: a leaked `kDate` is useless tomorrow, a leaked `kRegion` is useless outside `us-east-1`, and a leaked `kService` is useless for any service other than `s3`. This is why SigV4 keys can safely be embedded in a URL's query string without exposing the underlying long-term credential.
...
By inspecting the manual signing logic, it becomes clear how the URL binds authorization to a single resource and HTTP action:

- The **canonical request** hashes the exact HTTP method, path, and every query parameter together — changing any one of them (e.g. swapping `object_key`, or extending `X-Amz-Expires`) produces a different `canonical_request_hash`, which cascades into a different final signature.
- The **credential scope** (`date/region/service/aws4_request`) is baked into the string-to-sign, so a signature minted for `us-east-1` can never be replayed against a request that claims a different region.
- Since the signing key itself is re-derived fresh from the account's `Secret Access Key` on every request, there is no long-lived secret embedded anywhere in the URL — only a signature that is mathematically bound to this one request, at this one moment, for this one resource.

Any tampering with query parameters or expiration dates on the client side instantly invalidates the signature, forcing S3 to drop the request during execution validation.

## Key Takeaways

1. **Never sign with the raw Secret Access Key.** The four-step HMAC chain (date → region → service → `aws4_request`) scopes every derived key so a leak has a narrow blast radius.
2. **The canonical request is the anchor.** Every query parameter, HTTP verb, and header that participates in the canonical request is implicitly protected — mutate any of them and the signature fails.
3. **Understanding the math matters even when you use `boto3`.** The SDK's `generate_presigned_url()` performs exactly this derivation under the hood; knowing it lets you debug signature mismatches (clock skew, wrong region, extra headers) instead of guessing.
