...
description: "A repeatable review protocol for AI-authored diffs that targets the failure modes unique to LLM code generation — hallucinated APIs, plausible-but-wrong patterns, subtle logic gaps, and over-confident comments."
...
```text
[Read the diff without running it]
        |
        v
Does anything look too generic or too
clean for this specific codebase?
   |Yes                    |No
   v                        v
[Flag pattern-mismatch —    [List every external API call,
 compare to existing        method, and parameter introduced]
 convention]                        |
                                     v
                    [Verify each against real docs/signatures
                     — not memory, not the agent's comment]
                          ...
                          |Yes             |No
                          v                 v
              [Reject / request fix —  [Trace requirement to implementation:
               likely hallucination]    does this match INTENT, not just
                                         the prompt text?]
                                                |
                                                v
                                        [Run it — execute, don't just read]
                                                |
                                                v
                                  [Audit the tests: would they fail
                                   if the logic were subtly wrong?]
                                                |
                                                v
                                  [Check blast radius: does the diff
                                   touch only what it should?]
                                                |
                                                v
                          [Explicitly verify security-sensitive paths:
...
This connects directly to a broader pattern: an agent may never have "seen" your specific internal API in the first place — if the relevant type definitions or documentation weren't retrieved into context, the model is generating blind, filling the gap with the closest thing it has actually seen. And once a hallucinated fact is accepted into an agent's working state, it becomes context for the next reasoning step — errors compound across a longer agentic run in ways that make late-stage output look even more confident, not less.
...
For the agent-security surface itself (tool permissions, prompt injection, credential exposure during execution), see the companion article on MCP security — that article covers what the agent can do while working; this one covers what you verify after it's done.
