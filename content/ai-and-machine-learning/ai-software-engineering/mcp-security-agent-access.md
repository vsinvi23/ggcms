...
description: "How indirect prompt injection hijacks MCP-connected agents into destructive tool calls, and a zero-trust policy middleware that sandboxes paths, blocks command injection, and enforces human approval."
...
Because LLMs mix control instructions and data parameters in a single, unstructured text stream, standard network security controls are insufficient. Security must be enforced directly at the runtime protocol boundary.

## Architectural Hardening: The Zero-Trust Tool Registry

To prevent hijacked agents from causing damage, implement a zero-trust defense-in-depth model at the MCP client/server handshake:
...
Below is a security middleware that wraps an MCP tool execution engine. It intercepts outgoing tool calls, applies strict regex-based and command-sanitization blacklists, and enforces a manual confirmation prompt for critical, state-changing actions.
...
if __name__ == "__main__":
    middleware = MCPSecurityMiddleware(allowed_directories=["/workspace", "/home/shared"])
    ...
    success, resp = middleware.inspect_and_execute("read_file", {"path": "/workspace/../etc/passwd"}, read_file)
...
The path-traversal check and the command-character blacklist are the first line of defense — they run before the human ever sees a prompt, and they alone stop the most common indirect-injection payloads (`../` traversal, `;`/`|` chaining) without needing a human in the loop for every call. The human confirmation step is reserved for the smaller set of calls that pass sanitization but are still inherently high-risk (arbitrary shell execution).

## Key Takeaways

1. **Never trust data resources.** Ingesting text via MCP can hijack the agent's goal-seeking process.
2. **Isolate the execution context.** Tools should run in sandboxed containers, and file read/write operations must be restricted using strict canonical path checks.
