...
description: "How to build a pause-and-resume state machine that suspends AI agent execution at high-risk boundaries and resumes only after signed human approval."
...
When deploying AI agents in production environments, granting them direct write access to external systems — database modification, email transmission, financial transfers — presents severe operational risks. If an agent encounters a hallucinated state or a prompt injection attack, it may execute catastrophic, irreversible tool calls.
...
The following Python implementation uses a transition-validated state machine to manage human approval gates. The class serializes execution parameters, writes them to a persistent layer, and pauses execution until a signed webhook resumes the flow.
...
A minimal exercise of the state machine end to end:

```python
def send_wire_transfer(tool_name: str, args: Dict[str, Any]) -> str:
    return f"Transferred ${args['amount']} to {args['account']}"

workflow = HumanInTheLoopWorkflow()

# Agent attempts a high-risk action -> pauses instead of executing
paused = workflow.execute_step(send_wire_transfer, "wire_transfer", {"amount": 50000, "account": "ACC-1"}, risk_level="HIGH")
print(paused)  # {'status': 'paused', ...}

serialized = workflow.serialize_state()  # persist to DB / queue here

# ... later, a signed webhook arrives from the approval UI ...
restored = HumanInTheLoopWorkflow()
restored.load_state(serialized)
result = restored.receive_intervention(approved=True)
final = send_wire_transfer(result["args"]["tool"] if "tool" in result["args"] else "wire_transfer", result["args"])
```
...
1. **Treat risk boundaries as state transitions, not exceptions.** A high-risk tool call should deterministically move the workflow into `PENDING_APPROVAL`, not just log a warning and proceed.
2. **Persist enough state to resume cold.** Because approval can take hours, the paused state must be fully serializable and reloadable by a different process instance than the one that paused it.
3. **Approval without cryptographic non-repudiation is not a security control.** A plain "yes" click in a UI is not sufficient authorization for irreversible actions — sign it.
