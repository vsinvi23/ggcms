...
description: "Why an LLM cannot directly execute an API call, how grammar-based decoding constrains tool-call generation to a JSON schema, and a from-scratch runtime execution engine that closes the loop safely."
...
An LLM is a next-token predictor. It has no network socket, no operating system, and no execution environment. It cannot make API calls. To design robust agentic systems, engineers must understand the boundary between text generation and runtime execution, and how tool schemas are parsed, validated, and enforced.
...
Below is a Python implementation showing how a client runtime manages this boundary manually. It parses the model's structured text output, maps it to a local function registry, handles execution, and returns the results to the context loop.
...
Note the critical security implication of `registry.execute`: because the LLM only ever produced a JSON *string*, the runtime — not the model — decides which Python function actually runs and with what arguments. This is the enforcement point where input validation, allow-listing, and permission checks belong; the model has no way to bypass it because it never had execution access in the first place.
