...
description: "Why naive file and shell tools exposed to an AI agent become remote-code-execution backdoors, and how to build a secure-by-design MCP server with schema validation, path canonicalization, and sandboxed execution."
...
Notice the order of operations in `secure_read_file`: the path is canonicalized and checked *before* any filesystem call touches it, the existence and type checks happen next, and only then is the file actually read — with a hard size cap enforced even on success. Each of these guards independently closes off a different exploit path: `safe_resolve_path` blocks `../../etc/passwd`-style traversal (including traversal hidden behind symlinks, since `.resolve()` follows them before the containment check runs), the type check blocks a request aimed at a device file or named pipe, and the size cap blocks a memory-exhaustion or token-budget-exhaustion attack via an oversized file.

## Key Takeaways

1. **Never trust raw path inputs.** Use Python's `.resolve()` and `Path.is_relative_to()` (or Node's `path.resolve()` combined with boundary checks) to guarantee filesystem isolation — do this before any file operation, not as an afterthought.
...
3. **Use official SDK frameworks.** Leverage structured schemas, typed BaseModel parameters, and standard handlers to benefit from built-in schema serialization and type enforcement instead of hand-rolling JSON-RPC parsing.
4. **Treat every MCP tool as a new authorization boundary**, not just a convenience wrapper — an LLM manipulated via prompt injection will use any wide-open tool exactly as ruthlessly as a human attacker would.
