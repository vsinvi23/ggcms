...
description: "How to wrap non-deterministic LLM API calls in the resilience patterns production systems require: schema validation, exponential backoff with jitter, model-tier fallback cascades, and a hard deterministic safety net."
...
This Python implementation leverages the `tenacity` retry framework's concepts (implemented here by hand for transparency) and `pydantic` validation models to guarantee structured data parsing, backed by a secondary fallback model if the primary model repeatedly fails.
...
Walking through `execute_and_validate`: it never lets a malformed response or an exhausted retry budget propagate as an unhandled exception all the way to the caller. Each tier — primary model, fallback model, hard deterministic default — is strictly less capable but strictly more certain to return *something* schema-valid, which is exactly the trade-off a production system needs: a slightly worse answer beats a crashed pipeline.
...
- Treat LLM API responses as untrusted, possibly-malformed input by default — validate every response against a strict schema (Pydantic or equivalent) before any downstream code touches it.
- Exponential backoff with jitter prevents a thundering-herd retry storm against a rate-limited upstream, while a max-retry ceiling prevents an agent from looping forever on a persistently failing call.
- A fallback cascade (primary model → secondary model → hard deterministic default) guarantees the pipeline always returns a schema-valid result, trading capability for certainty exactly when certainty matters most.
- Feeding the model its own validation error back as a reflection prompt is often cheaper and more effective than immediately escalating to a different model tier.
