...
description: "Why traditional infrastructure observability misses agent reasoning failures, and how to extend OpenTelemetry to trace LLM decisions, token cost, and tool execution in one unified trace."
...
### Detecting Reasoning-Loop Failures That Infra Metrics Miss

A workflow with normal HTTP 200s and normal CPU can still be silently failing at the cognitive layer. Two agent-specific signals worth alerting on that have no infrastructure analogue:

- **Prompt-token-to-completion-token ratio drift** — a sudden spike often means the agent is re-reading the same context repeatedly (a symptom of context drift or a stuck retry loop), not that the task genuinely got harder.
- **Repeated identical tool spans within one trace** — the same `tool.name` span appearing several times in a row within a single `orchestrator_loop` is the trace-level signature of an infinite tool-call loop, and should page independently of whether the request eventually "succeeds."

## Key Takeaways

1. **Infrastructure health and agent health are different signals.** A 200 status code proves the HTTP layer worked; it says nothing about whether the reasoning inside was correct or efficient.
2. **Instrument the cognitive layer as its own span type.** Treat `llm_reasoning_turn` as a first-class span with token and latency attributes, not just a black box between two tool calls.
3. **Never put raw prompts in span attributes.** Correlate via `TraceID`/`SpanID` into a separate, access-controlled log store instead.
