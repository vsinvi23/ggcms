...
description: "How to scope a legacy refactor so an AI agent can attempt it safely, build the characterization-test safety net that proves behavior didn't change, and avoid the Hyrum's Law regressions that make legacy refactoring uniquely dangerous."
...
Three things make legacy refactoring different from — and harder than — greenfield coding tasks.
...
Mapped onto the general agent loop, a legacy refactor task looks like this:
...
6. **Updated State**: on any failure, the agent's next action must be "revert and reconsider the transformation" — not "adjust the test to match the new output." An agent with write access to the test files can "fix" a failing characterization test by editing the recorded expectation instead of the code, which silently defeats the entire safety net. This is worth calling out explicitly in the agent's instructions and, ideally, enforcing structurally.
...
Large-scale legacy modernization efforts tend to converge on a small set of staged-rollout patterns precisely because "refactor everything, then flip over" is too risky to verify in one step, agent-assisted or not:
