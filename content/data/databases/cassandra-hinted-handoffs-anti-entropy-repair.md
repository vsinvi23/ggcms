---
title: "Cassandra Entropy: Hinted Handoffs, Anti-Entropy Repair, and Zombie Data"
description: "How Apache Cassandra reconciles data divergence after node outages using hinted handoffs for short gaps and Merkle-tree-based anti-entropy repair for long ones — and why skipping repairs resurrects deleted data."
type: "ARTICLE"
...
# Cassandra Entropy: Hinted Handoffs, Anti-Entropy Repair, and Zombie Data
...
Cassandra chooses availability. But this introduces **entropy** — divergence in the dataset between replica nodes. If Node C is offline for three hours, it misses thousands of updates. Cassandra requires mechanisms to eventually reconcile this entropy and restore consistency.
...
**The Mechanism:** Once the Coordinator detects via gossip protocol that Node C has returned, it streams the stored hints to Node C.

- **Time Limit:** Hints are not stored indefinitely. `max_hint_window_in_ms` defaults to 3 hours. If a node is down longer than this, the Coordinator deletes the hints. Why? Because storing unbounded hints would eventually fill up the Coordinator's disk, taking down healthy nodes.
...
```bash
# Trigger a full anti-entropy repair on a keyspace, scoped to the local datacenter
...
```text
Node A (Offline > gc_grace_seconds)   Node B (Online)         Node C (Online)
[Active Row, never deleted]           [Tombstone, then purged] [Tombstone, then purged]
              \                              |                        |
               \                             |                        |
                +----------> Read Query Merges Rows <-----------------+
                                     |
                                     v
                     Zombie Row Resurrected! (Node A's stale copy wins)
```

## Operational Rule of Thumb

The relationship between the repair cadence and `gc_grace_seconds` is not optional — it is a hard correctness requirement:

```text
repair_interval  <  gc_grace_seconds
```

If your Reaper/cron repair cadence is every 3 days, `gc_grace_seconds` must stay comfortably above 3 days (the 10-day default gives headroom). Only lower `gc_grace_seconds` after you have verified repairs are actually completing successfully and on schedule.
